...
if(sumTotal>0){
				...
				for(var k=0;k<myAjax.responseXML.documentElement.getElementsByTagName("a1").length;k++){
					...
					if(myAjax.responseXML.documentElement.getElementsByTagName("a1")[k].text!=undefined){
						...
						//checkSele(k);sql3
						//document.getElementById('sql3').innerHTML=myAjax.responseXML.documentElement.getElementsByTagName("sql")[0].textContent;
						checkSele(k);
						...
						//alert(myAjax.responseXML.documentElement.getElementsByTagName("a3")[k].textContent);
...
function detailEndorse(){//상세보기
	...
	if(eNum){
		...
		window.open('/2012/pop_up/cDaeriEndorse.php?daeriComNum='+daeriComNum+'&CertiTableNum='+CertiTableNum+'&eNum='+eNum+'&endorseDay='+endorseDay,'new_22Union','left='+winl+',top='+wint+',resizable=yes,width=930,height=540,scrollbars=yes,status=yes')
	}else{
		alert('조회 후 !!');
		return false;
	}
