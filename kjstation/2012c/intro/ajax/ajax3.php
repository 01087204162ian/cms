...
include '../../../dbcon.php';
	...
	if($s_contents){
		...
		$sql.="(a.wdate  >='$sigi' and a.wdate  <='$end') $where2 $where3 order by wdate desc";
		...
		if($chr!=11){
			...
			$where="WHERE (wdate  >='$sigi' and wdate  <='$end') and 2012DaeriCompanyNum='$cNum' $where2 $where3 order by wdate desc ";
...
echo"<data>\n";
	...
	for($k=$count;$k<$last;$k++){
	   ...
	   $a[2]=mysql_result($result2,$count,"2012DaeriCompanyNum");
			...
			//대리운전회사 찾기 
		$cSql="SELECT num,company FROM  2012DaeriCompany WHERE num='$a[2]'";
