...
<div data-role="page" id="list">
	...
	<div data-role="content">
			...
			<?for($j=0;$j<$Num;$j++){
				...
				<?if($row[push]==4){?>
					<p class="ui-li-aside">
					<a href="view.php?id=<?=$id?>&num=<?=$row[num]?>&endorse_day=<?=$now_time?>" data-icon="edit" data-inline="true" data-role='button'><?=$pushName?></a>
	...
	<div data-role="footer" data-theme="b" data-position="fixed">
		<div data-role="navbar">
			<ul>
			  ...
			  <li><a  href="seleCerti.php?com=<?=$row[InsuraneCompany]?>&id=<?=$id?>" data-icon="plus" data-theme="b" data-transition="flip"><?=$inSom[$_p]?></a></li>
			  <!-- 증권선택 -->
			<!--직접입력  <li><a data-icon="plus" data-theme="b" href="write.php?id=<?=$row[InsuraneCompany]?>&dNumber=<?=$id?>&CertiTableNum=<?=$row[num]?>" ><?=$inSom[$_p]?></a></li>
