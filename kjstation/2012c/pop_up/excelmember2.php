...
$where="WHERE 2012DaeriCompanyNum='$cNum' AND (InPutDay>='$start' AND InPutDay<='$end2') AND (push='4' or push='1')";
...
for($j=0;$j<$num;$j++){
	...
	$pSql="SELECT * FROM 2015dongbudailyP WHERE snai<='$row[nai]' and enai>='$row[nai]' and date='$row[InPutDay]'  order by num";
	...
	$g++;
	echo "$g	$row[InPutDay]	$row[Name]	$row[Jumin]	$dongbucerti	";
	 for($_k=0;$_k<$pnum;$_k++){

		 $pRow=mysql_fetch_array($pRs);
		 $toPreminum+=$pRow[preiminum];
		 $m2=number_format($pRow[preiminum]);

			echo "$m2	";

			
		}
		$total=number_format($toPreminum);
	echo "$total\n";
	$toPreminum='';
