...
//alert(myAjax.responseText);
			...
			if(myAjax.responseXML.documentElement.getElementsByTagName("message")[0].text!=undefined){
				...
				for(var j=0;j<eval(myAjax.responseXML.documentElement.getElementsByTagName("Rnum")[0].text);j++){
					...
					sele(j,myAjax.responseXML.documentElement.getElementsByTagName("certiNum"+j)[0].text);//상태를 만들기 위해
				...
				for(var j=0;j<eval(myAjax.responseXML.documentElement.getElementsByTagName("Rnum")[0].textContent);j++){
					...
					sele(j,myAjax.responseXML.documentElement.getElementsByTagName("certiNum"+j)[0].textContent);//상태를 만들기 위해
				//	document.getElementById('a4b'+j).value='';
...
function serch(val){
	...
	if(num){	
		var toSend = "./ajax/qboardAjaxSerch2.php?num="+num
				      +"&com="+com;
//alert(toSend);
...
function DaeriMemberCompany(){
    ...
    window.open('../pop_up/qboarMemberEndorse.php?DaeriCompanyNum='+cNum+'&InsuraneCompany='+com+'&CertiTableNum='+certi+'&gita='+gita,'ppss','left='+winl+',top='+wint+',resizable=yes,width=640,height=600,scrollbars=no,status=yes')
}
