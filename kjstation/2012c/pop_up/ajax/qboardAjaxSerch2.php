...
include '../../../dbcon.php';
	...
	$com =iconv("utf-8","euc-kr",$_GET['com']);
...
if($jumin){
	...
	$sql="SELECT * FROM 2012DaeriCompany WHERE jumin='$jumin'";
}
...
echo"<data>\n";
	...
	//증권번호 조회를 위하여 ...
	$certiSql="SELECT * FROM 2021CertiTable WHERE 2012DaeriCompanyNum='$num' and InsuraneCompany='$com' and startyDay>='$yearbefore' and control='1'"; //대리운전회사에 운전자 추가 할 수 있는 경우만
	//control 2이면 추가 불가
...
$inWonTotal=0;//초기화
	...
	for($_m=0;$_m<$Rnum;$_m++)
	{
			...
			switch($rCertiRow[divi])
		{
			...
			$gita[$_m]=$rCertiRow[gita];


			if(!$fstartyDay){ $fstartyDay=$FirstStartDay;}
			switch($gita[$_m]){
				case 1 :
					$gitaName[$_m]='일반'.$fstartyDay;

					break;
				case 2 :

					$gitaName[$_m]='탁송'.$fstartyDay;
					break;
				case 3 :

					$gitaName[$_m]='전탁송'.$fstartyDay;
					break;
			}
		...
		echo "<gita".$_m.">".$gita[$_m]."</gita".$_m.">\n";//
		echo "<gitaName".$_m.">".$gitaName[$_m]."</gitaName".$_m.">\n";//
