...
if($jumin){ // 주민번호로 조회 할때 num 값이 필요 
	...
	$message='핸드폰번호를 반드시 입력하셔야 합니다.   번호만 입력하시면 됩니다 !!';
...
//		echo "<name".$_u_.">".$a[$_u_]."</name".$_u_.">\n";//
 ...
 //   }
	...
	echo "<com>".$com."</com>\n";
if($com==2){//동부화재가 아닌 경우에만
	//증권번호 조회를 위하여 ...
	$certiSql="SELECT * FROM 2021CertiTable WHERE 2012DaeriCompanyNum='$num' and InsuraneCompany='$com'  ";
...
$inWonTotal=0;//초기화
	echo "<Rnum>".$certiSql.$Rnum."</Rnum>\n";
	for($_m=0;$_m<$Rnum;$_m++)
	{
			$rCertiRow=mysql_fetch_array($rs);

	
			$daeriInwonSql="SELECT num FROM 2021DaeriMember WHERE CertiTableNum='$rCertiRow[num]' and push='4'";
