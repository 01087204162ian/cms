...
$log_file = "./rate_export_log_" . date("Ymd_His") . ".txt";
$log_handle = fopen($log_file, "w");

// SQL 파일 생성
$sql_file = "./2019rate_export_" . date("Ymd_His") . ".sql";
$sql_handle = fopen($sql_file, "w");

// SQL 파일 헤더 작성
fwrite($sql_handle, "-- 2019rate 데이터 내보내기\n");
fwrite($sql_handle, "-- 생성일: " . date("Y-m-d H:i:s") . "\n\n");

// 테이블 생성 SQL
fwrite($sql_handle, "-- 테이블 구조\n");
fwrite($sql_handle, "CREATE TABLE IF NOT EXISTS `2019rate` (\n");
fwrite($sql_handle, "  `num` int(11) NOT NULL auto_increment,\n");
fwrite($sql_handle, "  `policy` varchar(12) NOT NULL default '',\n");
fwrite($sql_handle, "  `jumin` varchar(14) default NULL,\n");
fwrite($sql_handle, "  `rate` char(2) default NULL,\n");
fwrite($sql_handle, "  PRIMARY KEY  (`num`),\n");
fwrite($sql_handle, "  KEY `idx_policy` (`policy`),\n");
...
// 총 레코드 수 확인
$count_query = "SELECT COUNT(*) as total FROM `2019rate`";
...
// 테이블의 최소/최대 ID 확인
$range_query = "SELECT MIN(num) as min_id, MAX(num) as max_id FROM `2019rate`";
...
// 컬럼 이름을 SQL에 포함
$columns_sql = "INSERT INTO `2019rate` (";
...
while ($current_min_id <= $max_id) {
    ...
    $query = "SELECT * FROM `2019rate` WHERE num BETWEEN $current_min_id AND $current_max_id ORDER BY num";
    ...
    while ($row = mysql_fetch_assoc($result)) {
        ...
        // policy
        $values[] = (!empty($row['policy'])) ? "'" . mysql_real_escape_string($row['policy']) . "'" : "''";
