...
$log_file = "./kj_premium_export_log_" . date("Ymd_His") . ".txt";
$log_handle = fopen($log_file, "w");

// SQL 파일 생성
$sql_file = "./kj_insurance_premium_data_export_" . date("Ymd_His") . ".sql";
$sql_handle = fopen($sql_file, "w");

// SQL 파일 헤더 작성
fwrite($sql_handle, "-- kj_insurance_premium_data 데이터 내보내기\n");
fwrite($sql_handle, "-- 생성일: " . date("Y-m-d H:i:s") . "\n\n");

// 테이블 생성 SQL
fwrite($sql_handle, "-- 테이블 구조\n");
fwrite($sql_handle, "CREATE TABLE IF NOT EXISTS `kj_insurance_premium_data` (\n");
fwrite($sql_handle, "  `id` int(11) NOT NULL auto_increment,\n");
fwrite($sql_handle, "  `policyNum` varchar(50) NOT NULL default '',\n");
...
fwrite($sql_handle, "  KEY `idx_cNum_rowNum` (`policyNum`,`rowNum`)\n");
...
// 총 레코드 수 확인
$count_query = "SELECT COUNT(*) as total FROM `kj_insurance_premium_data`";
...
// 테이블의 최소/최대 ID 확인
$range_query = "SELECT MIN(id) as min_id, MAX(id) as max_id FROM `kj_insurance_premium_data`";
...
// 컬럼 이름을 SQL에 포함
$columns_sql = "INSERT INTO `kj_insurance_premium_data` (";
...
while ($current_min_id <= $max_id) {
    ...
    $query = "SELECT * FROM `kj_insurance_premium_data` WHERE id BETWEEN $current_min_id AND $current_max_id ORDER BY id";
    ...
    while ($row = mysql_fetch_assoc($result)) {
        ...
        // policyNum
        $values[] = (!empty($row['policyNum'])) ? "'" . mysql_real_escape_string($row['policyNum']) . "'" : "''";
