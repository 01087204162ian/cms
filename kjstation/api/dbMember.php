...
$log_file = "./member_export_log_" . date("Ymd_His") . ".txt";
$log_handle = fopen($log_file, "w");

// SQL 파일 생성
$sql_file = "./2012Member_export_" . date("Ymd_His") . ".sql";
$sql_handle = fopen($sql_file, "w");

// SQL 파일 헤더 작성
fwrite($sql_handle, "-- 2012Member 데이터 내보내기\n");
fwrite($sql_handle, "-- 생성일: " . date("Y-m-d H:i:s") . "\n\n");

// 테이블 생성 SQL
fwrite($sql_handle, "-- 테이블 구조\n");
fwrite($sql_handle, "CREATE TABLE IF NOT EXISTS `2012Member` (\n");
...
fwrite($sql_handle, "  `jumin1` varchar(14) default NULL,\n");
...
// 총 레코드 수 확인
$count_query = "SELECT COUNT(*) as total FROM `2012Member`";
...
// 테이블의 최소/최대 ID 확인
$range_query = "SELECT MIN(num) as min_id, MAX(num) as max_id FROM `2012Member`";
...
// 컬럼 이름을 SQL에 포함
$columns_sql = "INSERT INTO `2012Member` (";
...
while ($current_min_id <= $max_id) {
    ...
    $query = "SELECT * FROM `2012Member` WHERE num BETWEEN $current_min_id AND $current_max_id ORDER BY num";
    ...
    while ($row = mysql_fetch_assoc($result)) {
        ...
        // email
        $values[] = (!empty($row['email'])) ? "'" . mysql_real_escape_string($row['email']) . "'" : "NULL";
        ...
        // wdate (날짜 필드)
        $values[] = (!empty($row['wdate']) && $row['wdate'] != '0000-00-00') ? "'" . $row['wdate'] . "'" : "'0000-00-00'";
        
        // inclu
        $values[] = (!empty($row['inclu'])) ? "'" . mysql_real_escape_string($row['inclu']) . "'" : "''";
        
        // kind
        $values[] = (!empty($row['kind'])) ? intval($row['kind']) : "2";
        
        // pAssKey
        $values[] = (!empty($row['pAssKey'])) ? "'" . mysql_real_escape_string($row['pAssKey']) . "'" : "''";
        
        // pAssKeyoPen
        $values[] = (!empty($row['pAssKeyoPen'])) ? "'" . mysql_real_escape_string($row['pAssKeyoPen']) . "'" : "''";
