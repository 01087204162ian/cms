		...
		if(sumTotal>0){
			...
			for(var k=0;k<cert.length;k++){
				...
				if(com_phone[k].text!=undefined){
					...
					checkSele(k,cName[k].text,oName[k].text,dNumber[k].text);
					checkPhone(k);
				}else{
...
function checkSele(k,cName,oName,dNumber){
	/*대리기사 조회*/
	...
	/*조회 버튼 */
		 var bButton=document.getElementById('conname'+k);
		 var j=k+1;
		 //bButton.className='input5';
		 //aButton.value=val1;
		 bButton.style.cursor='hand';
		 bButton.style.width = '90px';
		 bButton.innerHTML=cName;
		 bButton.onclick=popup_drive;
...
function serch(val){
	...
	myAjax=createAjax();
		...
		var toSend = "./ajax/driveSerch.php?sigi="+sigi
				   +"&p_ush="+p_ush
				   +"&page="+page
				   +"&end="+end;
		//alert(toSend)
