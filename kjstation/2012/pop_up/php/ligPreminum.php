...
<table>
	<tr>
		...
		<td colspan='3'> <?=$Inscompany?>화재 보험료 동일 <input type="checkbox" id="sameCheck" onclick='LigsamePcheck(<?=$InsuraneCompany?>)'></td>
	</tr>
	<tr>
	 <th>구분</th>
	 <th width='20%'>년령</th>
   ...
   <tr>
     <?if($_m==0){?><td rowspan='5'>일반</td><?}?>
	 <td><?=$Realnai2[$_m]?><?=$Realnai[$_m]?><BR>
	     <?=$nai[$_m]?><?=$nai2[$_m]?>
	 </td>
 ...
 <?}?><!--업체 화면에서는 sele가 3이므로 노출 안됨-->
		<table>
			...
			<tr>
			  ...
			  <td><input type="text" name="p25p" id="p25p"   class='textareP' ></td>
			  <td><input type="text" name="p26p" id="p26p"   class='textareP' ></td>
			  <td><input type="text" name="p31p" id="p31p"   class='textareP' ></td>
			  <td><input type="text" name="p45p" id="p45p"   class='textareP' ></td>
			    <td><input type="text" name="p50p" id="p50p"   class='textareP' ></td>
			</tr>
		<!--	<tr>
			...
			<tr>
			<td colspan='5' bgcolor="#ffffff" align='center'><input type="button" class="btn-b" style="cursor:hand;width:90;"  value="기준일 입력"  onFocus='this.blur()' onClick="LiggijunDay()"></td>
