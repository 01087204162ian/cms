...
// echo "_h $_h ";// echo "sql $sql <Br>";
   ...
   $data[gigan2] = $gigan2;				//보험기간
   $data[seiral]= $c[10];				//일련번호
   $data[Name] = $row[Name]." ".$a[2];		    //계약자
   $data[address]= "(".$a[28].")".$a[29].$a[30];		//주소
   $data[hphone] = $row[hphone];        //연락처
   $data[Email]= $a[5];			//e-mail
   $data[pName] = $row[Name];			//피보험자
   $data[preminum]= number_format($row[preminum])."원"; //총보험료
   $data[preminum2] =  number_format($row[preminum])."원"; //납입하실보험료
   $data[company]= $sRow[company];        //업소명
   $data[gubun] = $gubun;				  //구분	
   $data[giDan]= $giDan;			      //산출기초단위
   $data[gi] = $row[gi];				  //산술기초수
   $data[mojemol]=  "(".$a[28].")".$a[29].$a[30];		  //목적물 소재지
   $data[babheng]= $row[inputDay];		  //발행일


   // 템플릿 파일명  
  $tpl_file = './frm/policy.frm'; 
