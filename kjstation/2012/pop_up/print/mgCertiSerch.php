...
$Dsql="SELECT * FROM 2012DaeriMember WHERE InsuranceCompany ='2' and InPutDay ='$endorseDay' and push='1'";
//echo "Dsql $Dsql <br>";
	...
	for($_m=0;$_m<$DNum;$_m++)
	{
		...
		$ju=explode("-",$DRow[Jumin]);

		$j2=substr($ju[1],0,1);
		$driverJumin[$_m]=$ju[0]."-".$j2."******";
...
//echo  $driverPush[$_m] ;
		...
		$month6=date("Y-m-d ",strtotime("$now_time -6 month"));

		$tu=explode("-",$month6);

		$su=explode("-",$now_time);

		//$dcerrti[$_m]=$tu[0].$tu[1].$tu[2].$su[0].$su[1].$su[2];
		$dcerrti[$_m]="2014.09.11"."   2015.09.11";
