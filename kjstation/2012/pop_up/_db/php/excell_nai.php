...
/* 중복 검사 * 가입되어 있는 가 확인한다 */
	...
	$dSql="SELECT * FROM 2012DaeriMember WHERE Jumin='$jumin' and push='4' and CertiTableNum='$certiTableNum'";
...
$cSql="SELECT * FROM 2012Cpreminum WHERE CertiTableNum='$certiTableNum' order by sPreminum ";
//echo $cSql;
...
$userId=$_SESSION['ssID'];
	...
	//청약과 동시에 해지 처리 를 합니다
		...
		//
		$bsql="SELECT * FROM 2012DaeriMember WHERE Jumin='$jumin' AND push='1' AND sangtae='1' AND CertiTableNum='$certiTableNum' AND ";
    ...
    include "./php/yPreminum.php";
			...
			if($dongliStartDay){ //특저일 기준의 1/12 보험료 구하자
				//$dongliNaiPreminum=$endorsePreminum;
					$psql="SELECT * FROM 2012Cpreminum  WHERE (e_nai>='$nai2' and s_nai<='$nai2') ";
					$psql.="and CertiTableNum='$certiTableNum'";
					$pRs=mysql_query($psql,$connect);
					$pRow=mysql_fetch_array($pRs);
					$PreminumMonth=$pRow[mPreminum];
