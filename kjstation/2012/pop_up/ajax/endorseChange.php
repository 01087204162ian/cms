	...
	if($row[sangtae]==2){
		...
		include "./php/personRate.php";
					...
					if($cRow['moValue']==2){
						...
						include "./php/preminumNaiEndorse_2.php";
			...
			//문자보냄
				   ...
				   switch($push){
						case 2 :	
							if($Divi==1){ //정산분납인 경우
								$msg=$insuranceCompany.$etag__name.$row[Name]."님 해지기준일[".$DendorseDay."][".$po."]배".number_format($endorsePreminum);
							}else{
								$msg=$insuranceCompany.$etag__name.$row[Name]."님 해지기준일[".$DendorseDay."][".$po."]".number_format($endorsePreminum);
							}
							break;
						case 4 :
							if($Divi==1){ //정산분납인 경우		
								$msg=$insuranceCompany.$etag__name.$row[Name]."님 기준일[".$DendorseDay."][".$po."]월".number_format($PreminumMonth)."배".number_format($endorsePreminum);
							}else{
								$msg=$insuranceCompany.$etag__name.$row[Name]."님 기준일[".$DendorseDay."][".$po."]월".number_format($PreminumMonth)."배".number_format($endorsePreminum);
							}
							...
							if($inSuranceCom==4){
									...
									//echo $endDay__;
									$validEndDay=$endDay2__[0].$endDay2__[1].$endDay__[2];
			...
			echo"<data>\n";
				echo "<num>".iconv("utf-8","euc-kr",$body).$updateSj."</num>\n";
