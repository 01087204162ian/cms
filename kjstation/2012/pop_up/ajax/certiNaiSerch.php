...
////2012Certi의 num
	...
	$Csql="SELECT * FROM 2012Certi WHERE num='$num'";
	$Crs=mysql_query($Csql,$connect);
	$Crow=mysql_fetch_array($Crs);
	$policyNum=$Crow[certi];

	//2012Cpreminum table 증권번호에 해당하는 것이 있는가 부터 차자
		$Qsql="SELECT * FROM 2012Cpreminum WHERE policyNum='$policyNum'";
	...
	if(!$Qrow[policyNum]){
		...
		$insert="INSERT into 2012Cpreminum (policyNum) ";
		$insert.= "values ('$policyNum')";

		mysql_query($insert,$connect);
	}

	$DaeriCompanyNum=iconv("utf-8","euc-kr",$_GET['DaeriCompanyNum']);//
	$InsuraneCompany=$Qrow[insurance];
	...
	//나이구분하여 찾기
		...
		echo"<data>\n";
			...
			echo "<message>".$Qsql.$message."</message>\n";
