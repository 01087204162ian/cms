...
$Sql="SELECT  * FROM 2012CertiTable   WHERE num='$num'";
	...
	$mSql="SELECT num FROM 2012DaeriMember  WHERE nai>='48'  ";
	$mSql.="and 2012DaeriCompanyNum='$DaeriCompanyNum' and CertiTableNum='$num' and push='4'";
	$m3rs=mysql_query($mSql,$connect);
	$inwon[2]=mysql_num_rows($m3rs);


	$mSql="SELECT num FROM 2012DaeriMember    WHERE nai>='26' and nai<='34' and etag='2'";
	$mSql.="and 2012DaeriCompanyNum='$DaeriCompanyNum' and CertiTableNum='$num' and push='4' ";
	$m1rs=mysql_query($mSql,$connect);
	$inwonE[0]=mysql_num_rows($m1rs);
	...
	$monthsP[0]=$row[preminum1];//26세~34세 보험료
	$monthsP[1]=$row[preminum2];//35세~47세 보험료
	$monthsP[2]=$row[preminum3];//48세 보험료


	//일반 인원 합계

	$generalDaeriTotal=$inwon[0]+$inwon[1]+$inwon[2];
...
if($generalDaeriTotal>0){
	...
	$DaeriComP1=round($monthsP[0]*($inwon[0]/$generalDaeriTotal)+$monthsP[1]*($inwon[1]/$generalDaeriTotal)+$monthsP[2]*($inwon[2]/$generalDaeriTotal),-1);
//보험회사에 내는 보험료 
	$InsuP1=round(($a[0]/12)*($inwon[0]/$generalDaeriTotal)+($a[1]/12)*($inwon[1]/$generalDaeriTotal)+($a[2]/12)*($inwon[2]/$generalDaeriTotal),-1);
}
	
	
	$monthsEP[0]=$row[preminumE1];//26세~34세 보험료 탁송
	$monthsEP[1]=$row[preminumE2];//35세~47세 보험료 탁송
...
$InsuP=($InsuP1+$InsuP2);
   ...
   include "./php/monthDay.php";//일일보험료 산출 하기우해
		...
		if($row[yearP1]>0){
			...
			echo "<P34P>".number_format($monthsP[1])."</P34P>\n";
			...
			for($_u_=0;$_u_<$Period;$_u_++){
				...
				echo "<g".$_u_.">".$p3[$_u_]."</g".$_u_.">\n";//탁송보험료
				echo "<h".$_u_.">".$q2[$_u_]."</h".$_u_.">\n";//일반보험료35세~47세
				echo "<i".$_u_.">".$q3[$_u_]."</i".$_u_.">\n";//탁송보험료

				echo "<j".$_u_.">".$r2[$_u_]."</j".$_u_.">\n";//일반보험료48세
				echo "<k".$_u_.">".$r3[$_u_]."</k".$_u_.">\n";//탁송보험료
			 }
			 echo "<eco>"."2"."</eco>\n";
