...
if($startyDay<"2022-01-01"){

		 if($e[15]<35){   $yPreminum=$rRow[preminun25]*10; }
		 if($e[15]>=35 && $e[15]<=44){   $yPreminum=$rRow[preminun44]*10; }
		 if($e[15]>=45 && $e[15]<=47){   $yPreminum=$rRow[preminun49]*10; }
		 if($e[15]>=48 && $e[15]<=54){   $yPreminum=$rRow[preminun50]*10; }
        // if($e[15]>=56 && $e[15]<=65){   $yPreminum=$rRow[preminun60]*10; }
		 if($e[15]>=55){   $yPreminum=$rRow[preminun60]*10; }

}else {
	

		 if($e[15]<29){   $yPreminum=$rRow[preminun25]*10; }
		 if($e[15]>=29 && $e[15]<=40){   $yPreminum=$rRow[preminun44]*10; }
		 if($e[15]>=41 && $e[15]<=49){   $yPreminum=$rRow[preminun49]*10; }
		 if($e[15]>=50  && $e[15]<=55){   $yPreminum=$rRow[preminun50]*10; }
         if($e[15]>=56 && $e[15]<=65){   $yPreminum=$rRow[preminun60]*10; }
		 if($e[15]>=66){   $yPreminum=$rRow[preminun66]*10; }
...
//년간보험료를 구하기 위해
	/*		$hsql="SELECT * FROM 2012Cpreminum  WHERE ePreminum>='$e[15]' and sPreminum<='$e[15]' ";
		...
		//echo"qhgja";	echo $e[30]; echo "<br>"; */
		  switch($e[30]){//보험회사에 따라
				...
				case 4 :   // 현대
						...
						if($nabang_1==1){
							  $daumPreminum=$yPreminum*0.8; //1회
						}else if($nabang_1==2){
							  $daumPreminum=$yPreminum*0.70;//2회까지 내었다면 1회20%,2회15%,남은 60%
						}else if($nabang_1==3){
							   $daumPreminum=$yPreminum*0.6;//3회까지 내었다면 1회25%,2회15%,3회15%남은 45%
						}else if($nabang_1==4){
							   $daumPreminum=$yPreminum*0.5;//4회까지 내었다면 1회25%,2회15%,3회15%,4회15% 남은 30%
		...
		//echo $daumPreminum;
	//	$thisPreminum=($afterPreminum-$daumPreminum); //

	$thisPreminum=round((($afterPreminum-$daumPreminum)*$personRate3),-1); //2024-09-21 수정함
