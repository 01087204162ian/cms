...
$eSql="SELECT * FROM 2021EndorseList WHERE CertiTableNum='$CertiTableNum' and pnum='$eNum' ";
...
//echo $eSql;
	...
	$eRow=mysql_fetch_array($eRs);
		...
		if($eRow[ch]==11){
		  $qupate="UPDATE 2021EndorseList SET ch='1' WHERE num='$eRow[num]'";
...
/*	$a[8]=$row[fax];
	...
	//증권번호 조회를 위하여 ...
	$certiSql="SELECT * FROM 2021CertiTable WHERE num='$CertiTableNum'";
...
///	
	$Dsql="SELECT * FROM 2021DaeriMember WHERE CertiTableNum='$CertiTableNum' and EndorsePnum='$eNum' AND ";
	...
	//if(!$e[15]){//나이 계산을 위해 
		...
		if($a[30]==2){// DB 화재 인경우
			...
			//2012.09.11일이 보험 나이  계산 기준일이 된다
			    ...
			    $mSql="SELECT * FROM 2021CertiTable WHERE num='$CertiTableNum'";
			...
			if($e[30]==3){
					...
					$rSql="SELECT * FROM  2021Certi  WHERE certi='$e[12]'";
		...
		//}else{
			$tupdate="UPDATE 2021DaeriMember SET nai='$p[0]' WHERE num='$e[3]'";
...
//
	...
	echo "<Endorse2Preminum>".$endorsePre[$k]."</Endorse2Preminum>\n";//
	//echo "ㅂㅐ서 $endorsePreminum";
		//2018-11-29
		//이파리인 경우 
		// 자회사는 기존 대로 모회사 기준 보험료 구하이기 위해 
		if($mRow['moValue']==2){
			$m_sql="SELECT * FROM 2021CertiTable WHERE policyNum='$mRow[policyNum]' AND moValue='1'";
			//echo $m_sql; echo "<br>";
	...
	if($e[9]==13 || $e[9]==12){//가입한 다른보험회사가 있나 찾는다
			...
			$Csql="SELECT * FROM 2021CertiTable WHERE 2012DaeriCompanyNum='$num' ";
	...
	//이것을 통해 보험횟를 찾는다
		...
		if($e[10]!='9999'){
			$C2sql="SELECT * FROM 2021CertiTable WHERE num='$e[10]'";
			//echo "C  $C2sql <br>";
