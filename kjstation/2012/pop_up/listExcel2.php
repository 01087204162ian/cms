...
$sSql="SELECT * FROM 2012DaeriMember WHERE  push='4' $where1  $where2 $where3 order by CertiTableNum asc,Jumin asc";
...
<tr style=\"text-align:center;mso-number-format:'\@';\">  
   <td>메모</td>  
   <td colspan='10' style=\"text-align:center;color:red;mso-number-format:'\@';\">".$rowM['memo']."</td>    
</tr>
<tr>  
   <td>구분</td>  
   <td>성명</td>  
...
if($yRow[sigi]>'2019-07-31'){  // 2018 년 08월01일부터 나이구분이 변경됨
			...
			case 5 :
				...
				$yRow=mysql_fetch_array($yrs);
							 ...
							 if($sRow[nai]>=26 && $sRow[nai]<=36){  
								 $yPreminum[$j]=round($yRow[preminun25]*10/12,-1); 
								 $mY[$j]=$yRow[preminun25]; //보험회사 정규 보험료
							 }
							 if($sRow[nai]>=37 && $sRow[nai]<=57){ 
								 $yPreminum[$j]=round($yRow[preminun44]*10/12,-1); 
								 $mY[$j]=$yRow[preminun44]; //보험회사 정규 보험료
							 }
...
//	if($sRow[InsuranceCompany]==3){ //kb화재만 적용 
		if($pRow[divi]==2){ // 1/12 납인 경우에만
			...
			if($normalT==1){  //2022-03-05 표시를 할 경우 
				...
				//$p[$j]= $mY[$j]*$personRate2;
				//$q[$j]=$mY[$j]*$personRate2;//보험사 내는 1/12기준 보험료
			}else{
				$p[$j]='';
				$q[$j]='';
			}
...
/*	}else{
   ...
   <tr >  
	   ...
	   <td style=\"text-align:center;mso-number-format:'\@';\">".$policy[$j]."</td>
		...
		<td style=\"text-align:center;mso-number-format:'\@';\">".$personRateName."</td>
...
//증권별로  배서 기록을 찾아서 추가자 인원수 찾기
			...
			$e_sql.="and (endorse_day>='$mstart' and endorse_day<='$estart') and a.dagun='1' order by SeqNo desc";
