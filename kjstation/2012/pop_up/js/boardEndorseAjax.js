...
//alert(myAjax.responseXML.documentElement.getElementsByTagName("total")[0].text);
			...
			if(myAjax.responseXML.documentElement.getElementsByTagName("message")[0].text!=undefined){
				...
				{	document.getElementById('B1b'+_m).innerHTML=_m+1;
					...
					//alert(myAjax.responseXML.documentElement.getElementsByTagName("memberNum"+_m)[0].text);
					document.getElementById('B20b'+_m).value=myAjax.responseXML.documentElement.getElementsByTagName("startyDay2")[_m].text;
					document.getElementById('B21b'+_m).value=myAjax.responseXML.documentElement.getElementsByTagName("personRate")[_m].text;
					...
					//document.getElementById('B17b'+_m).innerHTML=myAjax.responseXML.documentElement.getElementsByTagName("sPrem")[_m].text;//보험회사에 내는 보험료
					...
					//처리 미처리
					var samP=myAjax.responseXML.documentElement.getElementsByTagName("sPrem")[_m].text;//보험회사에 내는 보험료
				...
				{	document.getElementById('B1b'+_m).innerHTML=_m+1;
					...
					//alert(myAjax.responseXML.documentElement.getElementsByTagName("memberNum"+_m)[0].textContent);
					document.getElementById('B20b'+_m).value=myAjax.responseXML.documentElement.getElementsByTagName("startyDay2")[_m].textContent;
					document.getElementById('B21b'+_m).value=myAjax.responseXML.documentElement.getElementsByTagName("personRate")[_m].textContent;
					...
					//document.getElementById('B17b'+_m).innerHTML=myAjax.responseXML.documentElement.getElementsByTagName("sPrem")[_m].textContent;//보험회사에 내는 보험료
					...
					//처리 미처리
					var samP=myAjax.responseXML.documentElement.getElementsByTagName("sPrem")[_m].textContent;//보험회사에 내는 보험료
...
function samSungcheck(){
	...
	var endorseDay=document.getElementById('endorseDay').innerHTML;
	  ...
	  myAjax=createAjax();//청약번호 저장을 하기위해
		...
		var toSend = "./ajax/boardSamPremInput.php?sPrem="+pRem
				   +"&driverNum="+dirverNum
...
function jcheck(){//설계번호
	...
	if(this.value.length==4){
	  ...
	  myAjax=createAjax();//청약번호 저장을 하기위해
		...
		var  dirverNum=document.getElementById('B0b'+nn).value;
		var toSend = "./ajax/boardAppInput.php?appNumber="+pCerti
				   +"&driverNum="+dirverNum;
				  // +"&page="+page
				  // +"&tableNum="+tableNum;
		//alert(toSend)
...
function hyuncerticheck(){//증권번호
   ...
   if(eval(document.getElementById('a14').value)==3 || eval(document.getElementById('a14').value)==4){
	...
	var toSend = "./ajax/boardCertiChange.php?memberNum="+memberNum
...
//	var c_preminum=document.getElementById('B17b'+nn).innerHTML; //보험회사 보험료
	...
	var giJun__=document.getElementById("B20b"+nn).value;
	var giJun2__=document.getElementById("B21b"+nn).value;
	//giJun 값이 1이면 2019년10월31일 이후 증권에서는 
	//개인별 손해율이 값이 있는지 없는지 반드시 확인한다
	if(giJun__==1){
	...
	//설계번호.증권번호 저장을위해
		...
		if(memberNum){	
				...
				if(val==1){
					...
					var toSend = "./ajax/boardEndorseChange.php?memberNum="+memberNum
...
if(val){
	for(var k=0;k<9;k++){	
			...
			document.getElementById('B17b'+k).innerHTML='';
...
function changeEndorseDay(){
	...
	if(eNum){
	    var winl = (screen.width - 1024) / 2
		var wint = (screen.height - 768) / 2
		window.open('./php/boardEndorseDayChange.php?eNum='+eNum+'&CertiTableNum='+CertiTableNum+'&endorseDay='+endorseDay+'&num='+num,'ed','left='+winl+',top='+wint+',resizable=yes,width=600,height=400,scrollbars=no,status=yes')
	}
...
function dailyPreminum(){//보험회사별 일일보험료 
	...
	var emday=document.getElementById('endorseDay').innerHTML;
		 var winl = (screen.width - 1024) / 2
			...
			window.open('./boardNewPreminum.php?DaeriCompanyNum='+num+'&CertiTableNum='+certiNum+'&InsuraneCompany='+a14+'&policyNum='+a9+'&emday='+emday,'12preminum','left='+winl+',top='+wint+',resizable=yes,width=960,height=800,scrollbars=yes,status=yes')
		/*switch(eval(a14)){
			case 1 :
				 var winl = (screen.width - 1024) / 2
				var wint = (screen.height - 768) / 2
