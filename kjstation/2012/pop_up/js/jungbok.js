...
if(sumTotal>0){
				...
				for(var k=0;k<myAjax.responseXML.documentElement.getElementsByTagName("a1").length;k++){
					...
					if(myAjax.responseXML.documentElement.getElementsByTagName("a1")[k].text!=undefined){
						...
						var hphone=myAjax.responseXML.documentElement.getElementsByTagName("a29")[k].text;
						hpHone(k,hphone);
						...
						var hphone=myAjax.responseXML.documentElement.getElementsByTagName("a29")[k].textContent;
						hpHone(k,hphone);
...
function serch(val){
	...
	var s_contents=encodeURIComponent(document.getElementById("jumin").value);
	if(s_contents){
			myAjax=createAjax();
			switch(val){
				case 1 :/* 다음*/
			...
			var toSend = "../intro/ajax/juminSerch.php?s_contents="+s_contents
					   //+"&push="+push
					  // +"&kind="+kind
					   +"&page="+page;
...
function claimg(){  //사고 기록하기 위해 
	...
	if(nn.length==5){
		...
		var wint = (screen.height - 768) / 2
		//window.open('/kibs_admin/coProduce/pop_up/preminumSearchhyundai.php?hyundai_c_num='+ssang_c_num,'new_Union','left='+winl+',top='+wint+',resizable=yes,width=930,height=540,scrollbars=no,status=yes')
		window.open('../pop_up/Claim.php?daeriComNum='+daeriComNum+'&PolicyNum='+PolicyNum+'&DariMemberNum='+DariMemberNum+'&userId='+userId+'&jumin='+jumin,'claim','left='+winl+',top='+wint+',resizable=yes,width=930,height=800,scrollbars=yes,status=yes')
