...
function serch(val){
	...
	if(num){	
		...
		var toSend = "./ajax/boardCertiSerch.php?CertiTableNum="+CertiTableNum
			                                 +"&DaeriCompanyNum="+num;
											// +"&mNum="+mNum;
...
if(document.getElementById('a2b0').value){
		if(val==1){//신규 입력일때 
			var toSend = "./ajax/boardInsertMember.php?DaeriCompanyNum="+DaeriCompanyNum
			...
			var toSend = "./ajax/boardInsertMemberEndorse.php?DaeriCompanyNum="+DaeriCompanyNum
						   +"&CertiTableNum="+CertiTableNum
...
$(function(){
	$("#Excelup").click(function(){
		...
		window.open('./boardExcelUpload.php?CertiNum='+CertiNum+'&Dnum='+Dnum,'excelUp','left='+winl+',top='+wint+',resizable=yes,width=900,height=270, scrollbars=yes,status=yes');
...
function newInsert(){
	...
	//var insuranceComNum=document.getElementById('insuranceComNum').options[document.getElementById('insuranceComNum').selectedIndex].value;
	    ...
	    var winl = (screen.width - 1024) / 2
		var wint = (screen.height - 768) / 2
		window.open('./boardNewInseret.php?DaeriCompanyNum='+DaeriCompanyNum+'&CertiTableNum='+certiNum,'rlist','left='+winl+',top='+wint+',resizable=yes,width=400,height=600,scrollbars=no,status=yes')
