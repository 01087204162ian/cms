...
function serch(val){
	...
	if(s_contents){
			insuranceComNum=99;
		var toSend = "../intro/ajax/boardDaeriCompanySerch.php?s_contents="+s_contents
		...
		var toSend = "../intro/ajax/boardDaeriCompanySerch.php?CertiTableNum="+CertiTableNum
...
function inWonList(){
	...
	var insuranceComNum=document.getElementById('insuranceComNum').options[document.getElementById('insuranceComNum').selectedIndex].value;
	    ...
	    var winl = (screen.width - 1024) / 2
		var wint = (screen.height - 768) / 2
		window.open('./boardMemberList.php?DaeriCompanyNum='+DaeriCompanyNum+'&CertiTableNum='+certiNum+'&InsuraneCompany='+insuranceComNum,'plist','left='+winl+',top='+wint+',resizable=yes,width=400,height=600,scrollbars=no,status=yes')
