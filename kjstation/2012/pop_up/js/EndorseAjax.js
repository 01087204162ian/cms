...
//alert(myAjax.responseXML.documentElement.getElementsByTagName("total")[0].text);
			...
			if(myAjax.responseXML.documentElement.getElementsByTagName("message")[0].text!=undefined){
				...
				{	document.getElementById('B1b'+_m).innerHTML=_m+1;
					...
					if(myAjax.responseXML.documentElement.getElementsByTagName("Jumin")[_m].text){
...
function juminSet(k){
   	...
   	var newInput2=document.createElement("input");
			 var aJumin=document.getElementById('B3b'+k);
			 
			 newInput2.id='jUmin'+k;
			 newInput2.style.width = '140px';
			 newInput2.className='phone';
			// newInput2.value=samP;//보험회사에내는 보험료 설정을 하면 그것을 계산해서 한다.....
			newInput2.onblur=JuminRecord;
			newInput2.onclick=jumiN_check_2;
...
function jumiN_check_2(id,val){
	...
	var val=document.getElementById(this.id).value;
	  if(val.length=='14'){
			...
			document.getElementById(this.id).value=phone_first+phone_second;
	 }
}

function JuminRecord(){
	...
	if(val.length>1){
	  ...
	  myAjax=createAjax();//청약번호 저장을 하기위해
		...
		var toSend = "./ajax/juminInput.php?jumin="+document.getElementById(this.id).value
				   +"&driverNum="+dirverNum
