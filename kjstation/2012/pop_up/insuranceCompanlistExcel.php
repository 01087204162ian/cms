...
$damdanga=$row[MemberNum];
		...
		//$damName=$dsrow[name];
$output_file_name = "보험회사 보고용".$sigi;
	...
	$j=$i+1;
		...
		if($pRow[moValue]==2){
			...
			$dongbuCerti[$j]=$sRow[dongbuCerti];
				...
				//대리운전회사 명을 찾기 위해 
				$cs_Sql =" SELECT * FROM 2012DaeriCompany WHERE num='$e[50]'";
				//echo $cs_Sql;
				$cs_rs =mysql_query($cs_Sql,$connect);
				$cs_row =mysql_fetch_array($cs_rs);
...
/*	}else{
   ...
   <tr >  
	   ...
	   <td style=\"text-align:right;mso-number-format:'\@';\">".number_format($q[$j])."</td> 
		...
		<td style=\"text-align:center;mso-number-format:'\@';\">".$cs_row['company']."</td>
...
//증권별로  배서 기록을 찾아서 추가자 인원수 찾기
		...
		//	$e_sql.="and (endorse_day>='$mstart' and endorse_day<='$estart')  and qboard='2' order by SeqNo desc";
				...
				$e_sql="SELECT  *  FROM SMSData ";
			    $e_sql.=" WHERE push>='1' ";
			    $e_sql.="and (endorse_day>='$sigi' and endorse_day<='$end')  and qboard='2' order by SeqNo desc";
				//echo $j;echo $e_sql;
				...
				//	}
					...
					$s[50]=$e_row['2012DaeriCompanyNum'];//대리운전회사

				//대리운전회사 명을 찾기 위해 
				$cs2_Sql =" SELECT * FROM 2012DaeriCompany WHERE num='$s[50]'";
				//echo $cs_Sql;
				$cs2_rs =mysql_query($cs2_Sql,$connect);
				$cs2_row =mysql_fetch_array($cs2_rs);
				...
				$EXCEL_STR2 .= "  
				   <tr>  
					   ...
					   <td style=\"text-align:right;mso-number-format:'\@';\">".number_format($e_row[preminum])."</td>
						...
						<td style=\"text-align:right;mso-number-format:'\@';\">".$cs2_row['company']."</td>
				   </tr>  
				   ";  
				}
