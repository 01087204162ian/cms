...
function claimg(){  //사고 기록하기 위해 
	...
	if(nn.length==5){
		...
		var wint = (screen.height - 768) / 2
		//window.open('/kibs_admin/coProduce/pop_up/preminumSearchhyundai.php?hyundai_c_num='+ssang_c_num,'new_Union','left='+winl+',top='+wint+',resizable=yes,width=930,height=540,scrollbars=no,status=yes')
		window.open('../pop_up/boardClaim.php?daeriComNum='+daeriComNum+'&PolicyNum='+PolicyNum+'&DariMemberNum='+DariMemberNum+'&userId='+userId+'&jumin='+jumin,'claim','left='+winl+',top='+wint+',resizable=yes,width=930,height=800,scrollbars=yes,status=yes')
...
function changeSangtae()
{
	...
	if(eval(val)==2){
		...
		if(confirm("해지신청 !!")){
			...
			var toSend = "../pop_up/ajax/boardEndorse.php?DariMemberNum="+DariMemberNum
