...
switch($val){
	...
	case 2 :
		...
		break;
	case 3 :
			$msg='화재배상  보험료'.number_format($row[preminum]).$bankName.$row[bankNum].'['.$inCom.']화재';
			$message='입금요청문자발송';
		break;
	case 4 :
	...
	case 5 :
			...
			$msg=$inCom.'화재배상 증권번호'.$row[policyNum].'이며 증권은'.$row[email].'로 발송 되었습니다';
			$message='증권발송 ';
		break;
	case 6 :
			$msg='화재배상책임보험관련 사업자등록증,소방방재청공문 02-6442-6419 팩스 보내주세요';
			$message='팩스 재요청';
			$val=1;
		break;
	case 10 :
...
$update="UPDATE 2013dajoong SET dCh='$val' WHERE num='$num'";
