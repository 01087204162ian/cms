...
include '../../../dbcon.php';
	...
	if($manGi!=99){
			...
			if($s_contents){
				...
				//$sql="SELECT distinct(policyNum),InsuraneCompany,2012DaeriCompanyNum,nabang,nabang_1  ";
				$sql="SELECT distinct(policyNum) ";
				$sql.="FROM  2012CertiTable  WHERE   startyDay>='$yearbefore' and InsuraneCompany!='2' ";
				$sql.="order by startyDay asc ";
			}
...
echo"<data>\n";
	...
	for($k=$count;$k<$last;$k++){
	  ...
	  // $a[12]=mysql_result($result2,$count,"DaeriCompany");
	   ...
	   //증권번호별로 대리기사 인원을 찾기위해


	   $pSql="SELECT * FROM 2012DaeriMember WHERE dongbuCerti='$a[4]' and push='4'";
	   $pRs=mysql_query($pSql,$connect);
	   $pRowNum=mysql_num_rows($pRs);
	   ...
	   $cSql="SELECT * FROM 2012CertiTable WHERE policyNum='$a[4]'";
	   $cRs=mysql_query($cSql,$connect);
	   $cRow=mysql_fetch_array($cRs);

	
	   $a[3]=$cRow[InsuraneCompany];
	   $a[5]=$cRow[startyDay];
	   $a[6]=$cRow[nabang];
	   $a[7]=$cRow[nabang_1];
		
		$dNum=$cRow['2012DaeriCompanyNum'];
		//대리회사명을 찾기위해
			$dSql="SELECT company,MemberNum FROM 2012DaeriCompany WHERE num='$dNum'";
			$dRs=mysql_query($dSql,$connect);
			$dRow=mysql_fetch_array($dRs);
...
//gita 1 이면 일반  2이면 탁송  3은 
	...
	/*	if($a[5]<=$yearbefore){//기준일이 1년전 오늘 보다 작은 경우엔
	   ...
	   echo "<a8>".number_format($pRowNum)."</a8>\n";
	   echo "<a9>".$a[9]."</a9>\n";
	   echo "<a12>".$a[12]."</a12>\n";
