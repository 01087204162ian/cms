...
include '../../../dbcon.php';
	...
	$daeriCompanyNum=$_GET['daeriCompanyNum'];
...
//	if($chr!=99){
	...
	if($daeriCompanyNum==99){
		$daeriCompanyNum='undefined';
	}

	if($daeriCompanyNum!='undefined'){
	  	$where6="and b.num='$daeriCompanyNum' ";
	}
	if($s_contents){
		...
		$where="WHERE    a.sangtae='1' $where2  $where3  $whrer4 $where5 $where6 order by a.push desc, a.dongbuCerti  desc,a.Jumin asc,a.InPutDay   asc,a.OutPutDay   asc";
	}
...
echo"<data>\n";
	...
	for($k=$count;$k<$last;$k++){
	  ...
	  // echo $a[3];
		...
		$e[27]=mysql_result($result2,$count,"sPrem");//보험회사 내는 보험료
	   ...
	   $e[30]=$a[7]=mysql_result($result2,$count,"InsuranceCompany");
	   ...
	   $a[11]=mysql_result($result2,$count,"2012DaeriCompanyNum");
		...
		//모계약을 찾기 위해 
			...
			$qSql="SELECT * FROM 2012CertiTable  WHERE num='$a[13]'";
			$qRs=mysql_query($qSql,$connect);
			$qRow=mysql_fetch_array($qRs);
			$moNum[$k]=$moNum_=$qRow[moNum];
...
if($moNum_){
		//모계약의 certi번호를 찾아서 대리운전회사를 찾기위해 ....
			...
			$rSql="SELECT * FROM 2012CertiTable  WHERE num='$moNum_'";
...
//모계약의 certi 번호를 기사별로 저장하기 위해 
			...
			$UmDate="UPDATE 2012DaeriMember SET moCertiNum='$moNum_' WHERE num='$a[1]'";
...
if($a[5]==1){//청약일 때만 
		//중복여부를 찾기위해 
		$dSql="SELECT * from  2012DaeriMember  WHERE Jumin='$a[16]' and push='4' and dongbuCerti='$a[8]' ";
	...
	//중복여부를 찾기위해 
		$dSql="SELECT * from  2012DaeriMember  WHERE Jumin='$a[16]' and push='4'  ";
	...
	if(!$a[8]){
	 ...
	 //  if($a[5]==4){
	   	...
	   	$SUmDate="UPDATE 2012DaeriMember SET dongbuCerti='$a[8]' WHERE num='$a[1]'";
	...
	//   }
	   ...
	   $eSql="SELECT * FROM 2012EndorseList WHERE pnum='$eNum' and  CertiTableNum='$a[13]' order by num desc";
	  //echo "eSql $eSql <Br>";
	...
	//if($a[5]==1){
	   /// 최종 가입건을 찾기위행
	   $L_SQL="SELECT * From 2012DaeriMember  WHERE Jumin='$a[16]'  and push='2' order by num desc";
...
//2019-10-15 
	...
	//보험회사에 내는 보험료를 찾기위해 
$e[12]=$a[8];//개별증권 번호 // 보험사 내는 보험료를 구하기 위함
$e[15]=$p[0];//만나이
$e[5]=$a[16];
...
include "../../pop_up/ajax/php/personRate.php";
		if($a[7]==8){  //삼성화재
			include "../../pop_up/ajax/php/inSurancePreminum.php";
		}
		

		if($a[7]==3){  //Lig 화재 
			include "../../pop_up/ajax/php/inSurancePreminum3.php";
		}

		if($a[7]==4){  //현대해상에 내는 보험료 계산하기위해
			include "../../pop_up/ajax/php/inSurancePreminum4.php";
		}

		if($a[7]==5){  //현대해상에 내는 보험료 계산하기위해
			include "../../pop_up/ajax/php/inSurancePreminum5.php";
		}
		//$e[12]
