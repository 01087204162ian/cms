	   ...
	   //증권번호를 찾기 위해 
		if($a[7]==2){//
			...
			//2012.09.11일이 보험 나이  계산 기준일이 된다
			    ...
			    $mSql="SELECT * FROM 2021CertiTable WHERE num='$a[13]'";
			...
			if(!$a[8]){
				...
				//개별증권번호가 없으면 입력하게 ...
				$uCsql="UPDATE 2021DaeriMember  SET dongbuCerti='$pRow[policyNum]' WHERE  num='$a[1]'";
		...
		$tupdate="UPDATE 2021DaeriMember SET nai='$p[0]' WHERE num='$a[1]'";
...
if($a[5]==4){	//정상인 경우만 	
	   ...
	   //다른 증권을 찾기 위해  //
	   $sql="SELECT distinct(InsuraneCompany),policyNum FROM  2021CertiTable  WHERE  2012DaeriCompanyNum='$DaeriCompanyNum' and startyDay>='$yearbefore'";
