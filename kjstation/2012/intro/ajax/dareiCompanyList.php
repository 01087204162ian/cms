...
//	}
	...
	$sql="SELECT  distinct 2012DaeriCompanyNum   FROM 2012DaeriMember a left join 2012DaeriCompany  b ";
...
echo "<values>\n";
	...
	for($k=$count;$k<$last;$k++){
	    ...
	    $a[13]=mysql_result($result2,$count,"2012DaeriCompanyNum");

		//echo  $a[13];
		//모계약을 찾기 위해 

			$qSql="SELECT * FROM 2012DaeriCompany  WHERE num='$a[13]'";
			...
			echo("\t<item>\n");
				echo("\t\t<policynum>".$qRow[company]."</policynum>\n");
				echo("\t\t<num>".$qRow[num]."</num>\n");
			echo("\t</item>\n");
	   $count++;
