...
if(sumTotal>0){
				...
				for(var k=0;k<myAjax.responseXML.documentElement.getElementsByTagName("a1").length;k++){
					...
					if(myAjax.responseXML.documentElement.getElementsByTagName("a1")[k].text!=undefined){
						...
						document.getElementById('endorseDay'+k).value=myAjax.responseXML.documentElement.getElementsByTagName("a9")[k].text;
						var samP=myAjax.responseXML.documentElement.getElementsByTagName("sPrem")[k].text;//보험회사에 내는 보험료
						seongSelect(myAjax.responseXML.documentElement.getElementsByTagName("a14")[k].text,k,myAjax.responseXML.documentElement.getElementsByTagName("rate")[k].text,samP);
						...
						if(myAjax.responseXML.documentElement.getElementsByTagName("a16")[k].text){
							...
							jungbog(k,myAjax.responseXML.documentElement.getElementsByTagName("a16")[k].text);
						...
						document.getElementById('endorseDay'+k).value=myAjax.responseXML.documentElement.getElementsByTagName("a9")[k].textContent;
						samP=myAjax.responseXML.documentElement.getElementsByTagName("sPrem")[k].textContent;//보험회사에 내는 보험료;
						seongSelect(myAjax.responseXML.documentElement.getElementsByTagName("a14")[k].textContent,k,myAjax.responseXML.documentElement.getElementsByTagName("rate")[k].textContent,samP);
						...
						if(myAjax.responseXML.documentElement.getElementsByTagName("a16")[k].textContent){
						  // document.getElementById("A16a"+k).innerHTML=myAjax.responseXML.documentElement.getElementsByTagName("a16")[k].textContent;
						    //document.getElementById('A16a'+k).style.color='#E4690C';
						   jungbog(k,myAjax.responseXML.documentElement.getElementsByTagName("a16")[k].textContent);
						}
...
function seongSelect(Pna_b,k,rate__,samP){
	 ...
	 var opts3=sjInput4.options;
		...
		//opts3[i].value=i;
			...
			switch(i){
				...
				case 1 :
					opts3[i].text='1';
					opts3[i].value='1';//가입경력 1년 미만 3년간 사고건수 1년간 사고건수 무사고 경역
				break;
				case 2 :
					opts3[i].text='0.9';
					opts3[i].value='2'
				break;
				case 3 :
					opts3[i].text='0.925'; //가입경력 1년 이상 3년간 사고건수 0건 1년간 사고건수 0건 무사고  1년 이상
					opts3[i].value='3'
				break;
				case 4 :
					opts3[i].text='0.898'; //가입경력 1년 이상 3년간 사고건수 0건 1년간 사고건수 0건 무사고  2년 이상
					opts3[i].value='4'
				break;
				case 5 :
					opts3[i].text='0.889'; ////가입경력 1년 이상 3년간 사고건수 0건 1년간 사고건수 0건 무사고  3년 이상
					opts3[i].value='5'
				break;
				case 6 :
					opts3[i].text="1.074"; //가입경력 1년 이상 3년간 사고건수 1건 1년간 사고건수 0건 
				    opts3[i].value='6'
				break;
				case 7 :
					opts3[i].text='1.085'; //가입경력 1년 이상 3년간 사고건수 1건 1년간 사고건수 1건 
					opts3[i].value='7'
				break;
				case 8 :
					opts3[i].text='1.242'; //가입경력 1년 이상 3년간 사고건수 2건 1년간 사고건수 0건 
					opts3[i].value='8'
				break;
				case 9 :
					opts3[i].text='1.253'; //가입경력 1년 이상 3년간 사고건수 2건 1년간 사고건수 1건 
					opts3[i].value='9'
				break;
				case 10 :
					opts3[i].text='1.314'; //가입경력 1년 이상 3년간 사고건수 2건 1년간 사고건수 2건 
					opts3[i].value='10'
				break;
				case 11 :
					opts3[i].text='1.428'; ////가입경력 1년 이상 3년간 사고건수 3건이상 1년간 사고건수 0건 
					opts3[i].value='11'
				break;
				case 12 :
					opts3[i].text='1.435'; ////가입경력 1년 이상 3년간 사고건수 3건이상 1년간 사고건수 1건 
					opts3[i].value='12'
				break;
				case 13 :
					opts3[i].text='1.447'; ////가입경력 1년 이상 3년간 사고건수 3건이상 1년간 사고건수 2건 
					opts3[i].value='13'
				break;
				case 14:
					opts3[i].text='1.459'; ////가입경력 1년 이상 3년간 사고건수 3건이상 1년간 사고건수 3건 이상
					opts3[i].value='14'
				break;
...
//배서처리
		var sjInput3=document.createElement("select");
	 var sjJumin=document.getElementById('A54a'+k);
     sjInput3.id='esang'+k;
	 sjInput3.style.width = '60px';
	 sjInput3.className='selectbox';
	 sjInput3.onchange=endorsChange;//배서 처리
	 var opts3=sjInput3.options;
		opts3.length=3;
		for(var i=1;i<opts3.length;i++){
...
function samSungcheck(){
	...
	//alert(nn)
	var endorseDay=document.getElementById('endorseDay'+nn).value;

	  myAjax=createAjax();//청약번호 저장을 하기위해

		//var push=document.getElementById('sang'+nn).options[document.getElementById('sang'+nn).selectedIndex].value;//청약 또는 
		var  pRem=this.value;
		var  dirverNum=document.getElementById('num'+nn).value;
		
		var toSend = "../pop_up/ajax/samPremInput.php?sPrem="+pRem
				   +"&driverNum="+dirverNum
		...
		myAjax.onreadystatechange=changeEtagResponse_1;
		myAjax.send('');
}

function changeEtagResponse_1() {
		if(myAjax.readyState == 4 && myAjax.status == 200) {
			//alert(myAjax.responseText);
...
function changeRate(){
 ...
 // alert(document.getElementById('peck'+val1).innerHTML);
  //var driver_num=document.getElementById(driver_num).value;
	...
	if(val2>=1){
		if(confirm('처리  합니까?')){
			var toSend = "./ajax/rateChange.php?policy=" + policy
					+"&policy="+policy
					+"&jumin="+jumin
					+"&rate_="+val2;
			//alert(toSend);
...
function endorsChange(){  //배서치리
	...
	var eNum=document.getElementById("eNum"+val1).value;
	var rate_=document.getElementById("rate_"+val1).value;
	var val =document.getElementById("A4a"+val1).innerHTML; //청약 또는 해지
	...
	if(rate_<0){
		...
		alert('개인별 할인할증을 반드시 입력 후 배서 처리하세요!!');
			document.getElementById("rate_"+val1).focus();
			document.getElementById("esang"+val1).value=1;
			return false;
...
var c_preminum=document.getElementById('cPrem'+val1).value;
/*if(bank==0){
		alert('입금 은행 부터!!!')
	...
	//alert(val2)
      if(val2>=1){
		//if(confirm('처리  합니까?')){
			var toSend = "./ajax/ChangeEndorse.php?driver_num=" + driver_num
					+"&val="+val
					+"&eNum="+eNum
...
function  jungbog(k,jungbogBunho){

	var bButton=document.getElementById('A16a'+k);
		// bButton.className='input5';
		 //aButton.value=val1;
		 bButton.style.cursor='hand';
		// bButton.style.width = '300px';
		
		 bButton.innerHTML=jungbogBunho;
		 bButton.onclick=jungbogSerch;

}

function jungbogSerch(){//상세보기
	...
	var dongbuCerti=document.getElementById('B17b'+nn).innerHTML; 
	//alert(jumin)
	...
	if(jumin){
		...
		var wint = (screen.height - 768) / 2
		//window.open('/kibs_admin/coProduce/pop_up/preminumSearchhyundai.php?hyundai_c_num='+ssang_c_num,'new_Union','left='+winl+',top='+wint+',resizable=yes,width=930,height=540,scrollbars=no,status=yes')
		window.open('../pop_up/Jungbok.php?jumin='+jumin,'new_dongbuCerti','left='+winl+',top='+wint+',resizable=yes,width=990,height=540,scrollbars=yes,status=yes')
	}else{
		alert('주민번호가 없음');
		return false;
	}
    ...
    var sigi=document.getElementById("sigi").value;
		var end=document.getElementById("end").value;
			...
			//	document.getElementById("A13a"+i).innerHTML='';
				...
				document.getElementById("A54a"+i).innerHTML='';
				document.getElementById("A15a"+i).innerHTML='';
				document.getElementById("A16a"+i).innerHTML='';
				document.getElementById("B17b"+i).innerHTML='';
...
//alert(toSend)
	...
	myAjax.send('');
	  ...
	  certiList(sigi,end,'',s_contents,insuranceComNum,damdanja,page);  //증권번호 조회하기
	  daeriCompanyList_(sigi,end,'',s_contents,insuranceComNum,damdanja,page);  // 대리운전회 조회하기
...
//배서리스트의 대리운전회사를 조회하여 리스트로 만들기 위해 
//2022-11-06
function daeriCompanyList_(sigi,end,chr,s_contents,insuranceComNum,damdanja,page){
		$("#dj2").children().remove();
		var send_url = "./ajax/dareiCompanyList.php";
				//alert(s_contents);
	      $.ajax({
					type: "POST",
					url:send_url,
...
function dp_certiList2_(){
	   ...
	   var newInput2=document.createElement("select");
		// var aJumin=document.getElementById('B16b'+k);
		 ...
		 newInput2.style.width = '80px';
		 newInput2.className='kj';
		 newInput2.onchange=certiChange__;/// 대리운전회사  조회하기
		 //newInput2.onchange=DnumChange;/// 주간분석
		 var opts=newInput2.options;
		 opts.length=certi_max_num;
		for(var _i=0;_i<opts.length;_i++){	
...
/*	if(certi==99){
	...
	var winl = (screen.width - 1024) / 2
	var wint = (screen.height - 768) / 2
	window.open('../pop_up/endorseExcel_2.php?sigi='+sigi
							+'&sigi='+sigi
							+'&end='+end
							,'endExcel_1','left='+winl+',top='+wint+',resizable=yes,width=900,height=570,scrollbars=yes,status=yes');
}
...
function preminum(j){ //Eslbs DB 보험료 정리 하기 위해
	var insCom=$('#insCom'+j).val(); //보험회사
	var cNum=$('#cNum'+j).val(); //대리운전회사
	var num=$('#num'+j).val(); //daerimembernum
	var endorseDay=$('#A10a'+j).html();//배서기준일
