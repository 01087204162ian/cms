...
include '../../../dbcon.php';
	...
	switch($gi){
				case '1' :
		         $kibs_sql="SELECT num FROM kibs_list WHERE com_name='$search_name'";
		...
		$sql="Select num,kibs_list_num,policy,invoice,startLocation,destination,sigi,wdate,useruid,sele ";	
		$sql.="FROM kibs_list_explain $where";
...
echo"<data>\n";
	...
	for($k=0;$k<$last;$k++){
			...
			$wdate = mysql_result($result2,$count,"destination");//목적지
			$user_id	 = mysql_result($result2,$count,"useruid");
			$policy  = mysql_result($result2,$count,"policy"); //년간예상금액
			$invoice  = mysql_result($result2,$count,"invoice"); 
			$vessel  = mysql_result($result2,$count,"startLocation");//출발지
			$sele	= mysql_result($result2,$count,"sele");//상태
	   ...
	   echo"<cancel_certi>".$policy."</cancel_certi>\n";
	   echo"<vessel>".$vessel."</vessel>\n";
	   echo"<wdate>".$wdate."</wdate>\n";
	   
	   echo"<com_name>".$com_name."</com_name>\n";
	   echo"<user_id>".$user_id."</user_id>\n";
	   echo"<num>".$kibs_list_num."</num>\n";
	    echo"<sele>".$sele."</sele>\n";
