...
function searchReceive() {
	if(myAjax.readyState == 4 && myAjax.status == 200) {
		...
		if(sumTotal>0){	
			...
			var nCerti=myAjax.responseXML.documentElement.getElementsByTagName("new_certi");
			...
			for(var k=0;k<wdat.length;k++){
				...
				if(com[k].text!=undefined){
					...
					document.getElementById('newCert'+k).innerHTML=nCerti[k].text;
					...
					document.getElementById('newCert'+k).innerHTML=nCerti[k].textContent;
...
function checkSele(k){
	 /*삭제 버튼 */
		 ...
		 aButton.innerHTML='취소/변경';
		 aButton.onclick=cancelSujeong;
}
function serch(val){
		for(var k=0;k<20;k++){
			document.getElementById('company'+k).innerHTML='';
			document.getElementById('oldCert'+k).innerHTML='';
			document.getElementById('newCert'+k).innerHTML='';
	...
	var toSend = "./ajax/ajax3Search.php?sigi="+sigi
			   +"&end="+end
		       +"&page="+page
			   +"&gi="+gi
...
function cancelSujeong(){
	...
	window.open('./print/cancel.php?cal_num='+val,'ssang','left='+winl+',top='+wint+',resizable=yes,width=700,height=300,scrollbars=yes,status=yes')
}
