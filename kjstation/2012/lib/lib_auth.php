...
function loginchk($userId,$userseq,$userIp)
{
	...
	if(!$userId or !$userseq or (md5($userId."sj") !=$userseq) or $userIp!=$_SERVER["REMOTE_ADDR"])
	{
		return false;
	}else{
...
if (!loginchk($_SESSION[userId],$_SESSION[userseq],$_SESSION[userIp]))
{
	...
	{		
		header("Location: ../../login.php?url=".urlencode($PHP_SELF));
