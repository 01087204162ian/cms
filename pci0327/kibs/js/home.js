...
function loadHomeData() {
  ...
  fetch('https://kjstation.kr/api/customer/home_data.php', {
    method: 'POST',
    body: formData
  })
  .then(response => {
...
// 한글 이름 유효성 검사
function validateKoreanName(input) {
...
function toDayEndorse(cNum) {
  ...
  fetch('https://kjstation.kr/api/customer/home_data_endorse.php', {
    method: 'POST',
    body: formData
  })
  .then(response => {
...
function updateHomeEndorseUI(data) {
  ...
  data.data.forEach((item, index) => {
    ...
    const maskedJumin = item.Jumin ? 
      `${item.Jumin.substring(0, 6)}-${item.Jumin.substring(6, 7)}******` : 
      '정보 없음';
    
    // 전화번호 마스킹 처리
    const maskedPhone = item.Hphone ? 
      item.Hphone.replace(/(\d{3})-(\d{4})-(\d{4})/, '$1-****-$3') : 
      '정보 없음';
    switch(item.etag) {
      case "1": 
        serviceType = "대리"; 
	...
	const pType={"1":"청약중","4":"해지중"}
		...
		// 상태에 따른 badge 클래스
		const badgeColor = item.push === "4" ? "bg-warning" : (item.push === "1" ? "bg-primary" : "");
		...
		row.innerHTML = `
		  ...
		  <td data-label="핸드폰">${maskedPhone}</td>
		  ...
		  <td data-label="상태"><span class="badge ${badgeColor} small">${pushText}</span></td>
		`;
      ...
      <div class="card-body py-2">
        ...
        <div class="info-row mb-1">
          <div class="row">
            ...
            <div class="col-7 info-value">${maskedPhone}</div>
          </div>
        </div>
...
function toDayEndorseAfter(cNum) {
  ...
  fetch('https://kjstation.kr/api/customer/home_data_endorse_after.php', {
    method: 'POST',
    body: formData
  })
  .then(response => {
