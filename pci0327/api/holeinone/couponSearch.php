...
try {
    ...
    // 보험료 조회 추가
    $premium_params = array();
    $premium_sql = "SELECT premium 
                    FROM dambo 
                    WHERE 1=1";
                    
    // policy_number가 제공된 경우에만 조건 추가
    if (!empty($policy_number)) {
        $premium_sql .= " AND policy_number = :policy_number";
        $premium_params[':policy_number'] = $policy_number;
    }
    
    logDebug("보험료 조회 SQL: " . $premium_sql);
    logDebug("보험료 조회 파라미터: " . print_r($premium_params, true));
    
    // 보험료 쿼리 실행
    $premium_stmt = $pdo->prepare($premium_sql);
    $premium_stmt->execute($premium_params);
    $premium_results = $premium_stmt->fetchAll(PDO::FETCH_ASSOC);
    logDebug("보험료 조회 결과 수: " . count($premium_results));
    ...
    // 보험료 정보 추가
    $response['premium_data'] = $premium_results;
    if (!empty($policy_number)) {
        $response['message'] .= " policy_number '{$policy_number}'의 보험료 정보도 포함되었습니다.";
    } else {
        $response['message'] .= " 전체 보험료 정보도 포함되었습니다.";
    }
