 ...
 * 담보 정보 검색 처리 스크립트
 * 
 * POST 요청으로 전송된 검색 조건에 따라 담보 정보를 조회합니다.
 * 결과는 JSON 형태로 반환됩니다.
 * 
 * 경로: /api/holeinone/damboSearch.php
 */
...
$searchTerm = isset($_POST['sj']) ? $_POST['sj'] : '';
$policy_number = isset($_POST['policy_number']) ? $_POST['policy_number'] : '';
try {
    logDebug("DB 연결 시도...");
    ...
    $sql = "SELECT id, policy_number, collateral, coverage_limit, deductible, description,premium 
            FROM dambo 
            WHERE 1=1";
    ...
    // 검색어가 제공된 경우 검색 조건 추가
    if (!empty($searchTerm)) {
        $sql .= " AND (policy_number LIKE :searchTerm OR collateral LIKE :searchTerm)";
	...
	$response['sql'] = $sql;
    $response['message'] = '담보 정보가 성공적으로 조회되었습니다.';
