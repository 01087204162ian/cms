 ...
 * 결과는 JSON 형태로 반환됩니다.
 * 
 * 경로: /api/holeinone/clientCertiSearch.php
 */
...
try {
    ...
    $sql = "SELECT cert_id, cert_number, company_id, issue_date,product_id
            FROM certificate 
            WHERE 1=1";
    ...
    if (!empty($searchTerm)) {
        $sql .= " AND (cert_number LIKE :searchTerm OR company_id LIKE :searchTerm)";
        $params[':searchTerm'] = '%' . $searchTerm . '%';
    }
    
    // 정렬 방식 설정
    $sql .= " ORDER BY cert_id DESC";
