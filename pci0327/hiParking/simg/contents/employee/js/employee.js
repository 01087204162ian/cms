...
function initializeEmployeeScripts() {
	removeExistingEventListeners();
    console.log("📌 employeeList.js 초기화 시작");
	


    const selectEl = document.getElementById('cSelect');
    const searchInput = document.getElementById('search-school');
    
    const placeholderMap = {
        '1': '성명을 입력하세요',
        '2': '아이디를 입력하세요',
        '3': '숫자만 입력하세요',  // 전화번호 placeholder 변경
        '4': '담당자를 입력하세요',
        '5': '팀을 입력하세요',
        '6': '역할을 입력하세요',
        '7': 'level을 입력하세요'
...
function formatPhoneNumber(value) {
			...
			// 지역번호 02로 시작하는 서울 전화번호
			else if (numbers.startsWith('02')) {
				if (numbers.length === 9) { // 02-XXX-XXXX
					return numbers.replace(/(\d{2})(\d{3})(\d{4})/, '$1-$2-$3');
				} else if (numbers.length === 10) { // 02-XXXX-XXXX
					return numbers.replace(/(\d{2})(\d{4})(\d{4})/, '$1-$2-$3');
				}
			}
			// 지역번호 031, 032 등으로 시작하는 전화번호
			else if (numbers.length === 10) {
				if (/^0[3-6][1-5]/.test(numbers)) { // 지역번호가 031~065인 경우
		...
		// 기존의 initializeQuestionScripts 함수 내부의 이벤트 리스너 수정
		searchInput.addEventListener('input', function(e) {
			if (selectEl.value === '3') {  // 전화번호 모드일 때
				const cursorPosition = this.selectionStart;
				const currentLength = this.value.length;
				const formattedNumber = formatPhoneNumber(this.value);
				
				// 값이 변경된 경우에만 업데이트
				if (formattedNumber !== this.value) {
					this.value = formattedNumber;
					
					// 커서 위치 조정
					const newLength = this.value.length;
					const cursorOffset = newLength - currentLength;
					this.setSelectionRange(cursorPosition + cursorOffset, cursorPosition + cursorOffset);
				}
			}
		});

    if (selectEl && searchInput) {
        // select 변경 이벤트
        selectEl.addEventListener('change', function() {
            const selectedValue = this.value;
            searchInput.placeholder = placeholderMap[selectedValue] || '검색어를 입력하세요';
            ...
            selectEl.style.backgroundColor = '#f0f7ff';
            setTimeout(() => {
        ...
        // 초기 placeholder 설정
        searchInput.placeholder = placeholderMap[selectEl.value] || '검색어를 입력하세요';
        
        // 초기 선택된 옵션 스타일 적용
        const selectedOption = selectEl.options[selectEl.selectedIndex];
        if (selectedOption) {
            selectedOption.style.color = '#2b4c8a';
...
if (event.target.classList.contains("k_2_open-second-modal")) {
	event.preventDefault();
	const num = event.target.dataset.num;

	fetch(`/simg/api/employee/get_employee_details.php?id=${num}`)
		.then(response => response.json())
		.then(response => {
    
			if (response.success) {
				const modal = document.getElementById("second-modal");
				document.getElementById("usersNum_").value = response.data.id;
				
				document.getElementById("mem_id_").textContent = response.data.userid;
				document.getElementById("userName_").textContent = response.data.name; 
				document.getElementById("contact_").value = response.data.email;
				document.getElementById("damdangat").value = response.data.phone;
				document.getElementById("level_").value = response.data.level;
...
function saveContact_() {
    const contact_ = document.getElementById("contact_").value.trim(); // 입력 값
    const num = document.getElementById("usersNum_").value; // userTable num 값
    ...
    fetch(`/simg/api/employee/update_contact_.php`, {
        method: "POST",
        headers: {
            "Content-Type": "application/x-www-form-urlencoded",
        },
        body: `contact_=${encodeURIComponent(contact_)}&num=${encodeURIComponent(num)}`
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
...
// 페이지 로드 시 한 번만 이벤트 리스너 추가
document.getElementById("level_").addEventListener("change", function(e) {
    level_change(e.target.value);
});

function level_change(value) {
    console.log('선택된 레벨:', value);
    const userName = document.getElementById("userName").value;
    const num = document.getElementById("usersNum_").value;

    fetch(`/simg/api/employee/update_level.php`, {
        method: "POST",
        headers: {
            "Content-Type": "application/x-www-form-urlencoded",
        },
        body: `level=${encodeURIComponent(value)}&num=${encodeURIComponent(num)}&userName=${encodeURIComponent(userName)}`
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            ...
            // 모달 닫기 (모달 ID가 'levelModal'이라고 가정)
            const modal = document.getElementById('second-modal');
            if (modal) {
                modal.style.display = 'none';
                // 또는 Bootstrap 모달을 사용하는 경우:
                // $('#levelModal').modal('hide');
            }

            // 좌측 메뉴 재클릭 (메뉴 요소의 ID가 'employeeList'라고 가정)
            const employeeList = document.getElementById('employeeList');
            if (employeeList) {
                employeeList.click();
                // 또는 특정 메뉴 항목을 클릭하는 경우:
...
function saveDamdangat() {
    const phone = document.getElementById("damdangat").value;
    
	
    const usersNum_ = document.getElementById("usersNum_").value;

    

    // fetch 요청 중복 실행 방지
    fetch('/simg/api/employee/update_damdangat.php', {
			method: "POST",
			headers: { "Content-Type": "application/x-www-form-urlencoded" },
			body: `num=${encodeURIComponent(usersNum_)}&damdangat=${encodeURIComponent(phone)}`
		})
		.then(response => response.json())
		.then(data => {
...
function removeExistingEventListeners() {
    ...
    console.log("✅ employeeList.js 초기화 완료");
}
