...
function fieldList(){
	 ...
	 const pageContent = document.getElementById('page-content');
		...
		const fieldContents= `<div class="f-list-container">
										<!-- 검색 영역 -->
										<div class="f-list-header">
											<div class="f-left-area">
												<div class="f-search-area">
													...
													<input type="text" id="f-searchKeyword" class="f-searchKeyword" placeholder="검색어를 입력하세요" onkeypress="if(event.key === 'Enter') searchList()">
													<button class="f-search-button" onclick="searchList()">검색</button>
												</div>
											</div>
										...
										<div class="f-list-content">
											<div class="f-data-table-container">
...
function loadTable(page = 1, searchSchool = '', searchMode = 1) {
            ...
            .then(response => {
                ...
                if (!response.data || response.data.length === 0) {
					...
					`;
                        ...
                        rows += `<tr>
                            <td><a href="#" class="f-btn-link_1 open-second-modal" data-num="${item.num}">${index + 1}</a></td>
 ...
 function renderPagination(currentPage, totalPages) {
    ...
    // 이전 버튼 추가
    if (currentPage > 1) {
        pagination.innerHTML += `<a href="#" class="f-page-link" data-page="${currentPage - 1}">이전</a>`;
    } else {
        pagination.innerHTML += `<a href="#" class="f-disabled">이전</a>`;
    }
    ...
    for (let i = startPage; i <= endPage; i++) {
        pagination.innerHTML += `<a href="#" class="f-page-link ${i === currentPage ? "active" : ""}" data-page="${i}">${i}</a>`;
    }

    // 다음 버튼 추가
    if (currentPage < totalPages) {
        pagination.innerHTML += `<a href="#" class="f-page-link" data-page="${currentPage + 1}">다음</a>`;
    } else {
        pagination.innerHTML += `<a href="#" class="f-disabled">다음</a>`;
    }

    // 페이지 이동 이벤트 추가
    document.querySelectorAll(".f-page-link").forEach(link => {
...
/* 검색 버튼*/
function searchList() {
    const searchType = document.querySelector("#f-searchType").value;
    const searchKeyword = document.querySelector("#f-searchKeyword").value.trim();
    ...
    loadTable(1, searchKeyword, searchType);
}
