    ...
    const fieldContents = `
        <div class="kje-list-container">
            ...
            <div class="kje-list-header">
                <!-- 데스크탑 검색 영역 -->
                <div class="kje-left-area desktop-only">
                ...
                <!-- 모바일 검색 영역 -->
                <div class="kje-mobile-search-area mobile-only">
                    <div class="mobile-filter-row">
                        <select id="pushSangtaeList-mobile" class="mobile-select" onChange="f_kjEndorseloadTable()">
                    ...
                    <div class="mobile-filter-row">
                        <select id="certiList-mobile" class="mobile-select"></select>
                    </div>
                    <div class="mobile-filter-row">
                        <select id="daeriCompanyList-mobile" class="mobile-select"></select>
                    </div>
                    <div class="mobile-filter-row">
                        <div id='currentSituation-mobile' class="mobile-current-status"></div>
                    </div>
                </div>
            ...
            <div class="kje-list-content">
                <!-- 데스크탑 테이블 -->
                <div class="kje-data-table-container desktop-only">
                ...
                <!-- 모바일 카드 뷰 -->
                <div class="kje-mobile-card-view mobile-only">
                    <div id="kje-mobile-cards-container">
            ...
            <!-- 데스크탑 페이지네이션 -->
            <div class="kje-pagination desktop-only"></div>
            
            <!-- 모바일 페이지네이션 -->
            <div id="kje-mobile-pagination-container" class="mobile-pagination-container mobile-only">
                <ul id="kje-mobile-pagination" class="mobile-pagination"></ul>
            </div>
        </div>
    `;
...
function kjEndorseloadSearchTable(){
  // 여기에 fetch 코드 넣기
	...
	const sj='sj_'
	// API 요청
	fetch(`../kj/api/kjDaeri/endorseCertiSerarch.php?sj=${sj}`)
    .then(response => {
      if (!response.ok) {
...
// populateCertiList 함수 수정
function populateCertiList(data) {
    const selectElement = document.getElementById('certiList');
    const mobileSelectElement = document.getElementById('certiList-mobile');
    ...
    if (mobileSelectElement) {
        ...
        mobileSelectElement.appendChild(mobileDefaultOption);
    }
    ...
    if (mobileSelectElement) {
        const mobileDefaultOption2 = defaultOption2.cloneNode(true);
        mobileSelectElement.appendChild(mobileDefaultOption2);
    }
    ...
    data.data.forEach(item => {
        ...
        if (mobileSelectElement) {
            ...
            mobileSelectElement.appendChild(mobileOption);
        }
    });

    // 현재 상황 업데이트
    const currentSituation = document.getElementById('currentSituation');
    const mobileCurrentSituation = document.getElementById('currentSituation-mobile');
    const statusText = `청약:${data.pushCounts.subscription},해지:${data.pushCounts.termination},계:${data.pushCounts.total}`;
    
    currentSituation.innerHTML = statusText;
    if (mobileCurrentSituation) {
        mobileCurrentSituation.innerHTML = statusText;
    }
    ...
    selectElement.addEventListener('change', function() {
        ...
        if (selectedValue === '') {
            ...
            kjEndorseloadTable(1, selectedValue, '', '');
        } else {
...
function SearchDaeriCompany(){
  // 여기에 fetch 코드 넣기
	...
	const sj='sj_'
	// API 요청
	fetch(`../kj/api/kjDaeri/endorseCompanySerarch.php?sj=${sj}`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
...
function CompanyList(data) {
    ...
    const mobileSelectElement2 = document.getElementById('daeriCompanyList-mobile');
    ...
    if (mobileSelectElement2) {
        mobileSelectElement2.innerHTML = '';
    }
    ...
    if (mobileSelectElement2) {
        const mobileDefaultOption = defaultOption.cloneNode(true);
        mobileSelectElement2.appendChild(mobileDefaultOption);
    }
    ...
    data.data.forEach(item => {
        ...
        if (mobileSelectElement2) {
            const mobileOption = option.cloneNode(true);
            mobileSelectElement2.appendChild(mobileOption);
        }
    });
    ...
    if (mobileSelectElement2) {
        mobileSelectElement2.addEventListener('change', function() {
...
async function EndorseCurrentSituation(){
 ...
 formData.append('toDate',todayStr);
  ...
  try {
    // API 요청 전송
    const response = await fetch(`../kj/api/kjDaeri/EndorseCurrentSituation.php`, {
      method: "POST",
      body: formData,
    });
...
function todayEndorseloadSearchTable(endorseDay,dNum,policyNum,sort){
  ...
  // API 요청
  fetch(`../kj/api/kjDaeri/todayEendorseCertiSearch.php?endorseDay=${endorseDay}&dNum=${dNum}&policyNum=${policyNum}&sort=${sort}`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
...
async function dailyCheck() {
  ...
  try {
    // API 요청 전송
    const response = await fetch(`../kj/api/kjDaeri/dailyEndorseSearch2.php`, {
      method: "POST",
      body: formData,
    });
...
function processEndorseData(result, dateStr) {
  console.log("받은 데이터:", result); // 디버깅용
  ...
  result.data.forEach(item => {
    console.log("처리 중인 항목:", item); // 디버깅용
    console.log("etag 타입:", typeof item.etag, "값:", item.etag); // 디버깅용
    console.log("push 타입:", typeof item.push, "값:", item.push); // 디버깅용
    ...
    console.log("분류:", 
      isDaeri ? "대리" : "탁송", 
      isRegistration ? "가입" : isTermination ? "해지" : "기타"); // 디버깅용
  ...
  console.log("분류 결과:", {
    "대리 가입자": daeriRegistrations.length,
    "대리 해지자": daeriTerminations.length,
    "탁송 가입자": taksongRegistrations.length,
    "탁송 해지자": taksongTerminations.length,
    "보험료 합계": {
      "대리 가입": daeriRegPremium,
      "대리 해지": daeriTermPremium,
      "탁송 가입": taksongRegPremium,
      "탁송 해지": taksongTermPremium
    }
  }); // 디버깅용
...
function todayEndorsedNumloadSearchTable(endorseDay){
  ...
  // API 요청
  fetch(`../kj/api/kjDaeri/todayEendorseCompanySerarch.php?endorseDay=${endorseDay}`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
...
async function dailyEndorseRequest(page = 1, selectedDate = null,dNum,policyNum,sort) {
  ...
  try {
    ...
    // API 요청 전송
    const response = await fetch(`../kj/api/kjDaeri/dailyEndorseSearch.php`, {
      method: "POST",
      body: formData,
    });
...
function kjEndorseloadTable(page = 1, certiValue = currentCertiValue, daeriValue = currentDaeriValue, pushValue = currentPushSangtaeValue) {
    ...
    if (certiValue) {
        ...
        if (document.getElementById('daeriCompanyList-mobile')) {
            document.getElementById('daeriCompanyList-mobile').value = '';
        }
    }
    ...
    if (daeriValue) {
        ...
        if (document.getElementById('certiList-mobile')) {
            document.getElementById('certiList-mobile').value = '';
        }
    }
    ...
    fetch(`../kj/api/kjDaeri/endorse.php?page=${page}&limit=${itemsPerPage}&certi=${certiValue}&dNum=${daeriValue}&push=${pushValue}`)
        .then(response => response.json())
        .then(response => {
            // 데스크탑 테이블 업데이트
            updateDesktopTable(response);
            
            // 모바일 카드 뷰 업데이트
            updateMobileCardView(response);
            
            // 현재 상황 업데이트
            updateCurrentSituation(response.pushCounts);
            
            // 데스크탑 페이지네이션 업데이트
            kjRenderPagination(page, Math.ceil(response.totalRecords / itemsPerPage));
            
            // 모바일 페이지네이션 업데이트
            updateMobilePagination(page, Math.ceil(response.totalRecords / itemsPerPage));
        })
        .catch((error) => {
            console.error("Error details:", error);
            alert("데이터를 불러오는 중 오류가 발생했습니다.");
...
// 데스크탑 테이블 업데이트 함수
function updateDesktopTable(response) {
    const tableBody = document.querySelector("#kje-applicationList");
    let rows = "";
    
    if (!response.data || response.data.length === 0) {
        rows = `<tr><td colspan="19" style="text-align: center;">검색 결과가 없습니다.</td></tr>`;
    } else {
        response.data.forEach((item, index) => {
            ...
            const rateOptions = `
                <select class="kje-status-rate" data-id="${item.num}" id='rate-${item.num}' onchange="handleRateChangeSj(this, '${item.num}','${item.Jumin}','${item.dongbuCerti}')">
                    <option value="-1">선택</option>
                    <option value="1" ${item.rate == 1 ? "selected" : ""}>1</option>
                    <option value="2" ${item.rate == 2 ? "selected" : ""}>0.9</option>
                    <option value="3" ${item.rate == 3 ? "selected" : ""}>0.925</option>
                    <option value="4" ${item.rate == 4 ? "selected" : ""}>0.898</option>
                    <option value="5" ${item.rate == 5 ? "selected" : ""}>0.889</option>
                    <option value="6" ${item.rate == 6 ? "selected" : ""}>1.074</option>
                    <option value="7" ${item.rate == 7 ? "selected" : ""}>1.085</option>
                    <option value="8" ${item.rate == 8 ? "selected" : ""}>1.242</option>
                    <option value="9" ${item.rate == 9 ? "selected" : ""}>1.253</option>
                    <option value="10" ${item.rate == 10 ? "selected" : ""}>1.314</option>
                    <option value="11" ${item.rate == 11 ? "selected" : ""}>1.428</option>
                    <option value="12" ${item.rate == 12 ? "selected" : ""}>1.435</option>
                    <option value="13" ${item.rate == 13 ? "selected" : ""}>1.447</option>
                    <option value="14" ${item.rate == 14 ? "selected" : ""}>1.459</option>
                </select>
            `;
            
            const MemberNumType = {"1": "sj", "2": "kj"};
            const Member = MemberNumType[item.MemberNum] || "알 수 없음";
            ...
            rows += `<tr>
                ...
                <td>${item.jumin2 || ""}</td>
                <td>${item.Hphone || ""}
                  <button type="button" class="sms-button sms-icon-button" title="문자 발송" onClick="sms('${item.Hphone}','${item.Name}','${item.num}');">SMS</button>
                </td>
                <td>${progressStage}</td>
                ...
                <td><span class="kje-btn-link_1" style="cursor:pointer;" onclick="kjMemberList('${item.Jumin || ''}',2)">${item.duplNum || ""}</span></td>
            </tr>`;
        });
    }
...
// 모바일 카드 뷰 업데이트 함수
function updateMobileCardView(response) {
    const cardsContainer = document.getElementById('kje-mobile-cards-container');
    ...
    response.data.forEach((item, index) => {
        ...
        card.className = 'kje-mobile-card';
        
        card.innerHTML = createMobileCardContent(item, index + 1);
...
// 모바일 카드 내용 생성 함수
function createMobileCardContent(item, index) {
    const formattedInsurancePremium = item.Endorsement_insurance_company_premium ? 
        parseFloat(item.Endorsement_insurance_company_premium).toLocaleString("en-US") : "";
    const formattedMothlyPremium = item.Endorsement_insurance_premium ? 
        parseFloat(item.Endorsement_insurance_premium).toLocaleString("en-US") : "";
    ...
    let endorseDayStyle = '';
    if (item.endorse_day && item.endorse_day !== todayStr) {
        endorseDayStyle = 'style="color: red; font-weight: bold;"';
    }
    ...
    return `
        ...
        <div class="card-body">
            ...
            <div class="info-row">
                <span class="info-label">대리운전회사:</span>
                <span class="info-value">${item.company || '-'}</span>
            </div>
            
            <div class="info-row">
                <span class="info-label">주민번호:</span>
                <span class="info-value">${item.jumin2 || '-'}</span>
            </div>
            
            <div class="info-row">
                <span class="info-label">핸드폰:</span>
                <span class="info-value">${item.Hphone || '-'}</span>
            </div>
            
            <div class="info-row">
                <span class="info-label">진행단계:</span>
                <span class="info-value">
                    <select class="mobile-progress-select" data-id="${item.num}" id='mob-progress-${item.num}' 
                            onchange="handleProgress(this, '${item.num}')">
                        <option value="-1">선택</option>
                        <option value="1" ${item.progress == 1 ? "selected" : ""}>프린트</option>
                        <option value="2" ${item.progress == 2 ? "selected" : ""}>스캔</option>
                        <option value="3" ${item.progress == 3 ? "selected" : ""}>고객등록</option>
            ...
            <div class="info-row">
                ...
                <span class="info-value" ${endorseDayStyle}>${item.endorse_day || '-'}</span>
            </div>
            
            <div class="info-row">
                <span class="info-label">신청일:</span>
                <span class="info-value">${item.wdate || '-'}</span>
            </div>
            
            <div class="info-row">
                <span class="info-label">증권번호:</span>
                <span class="info-value">${item.dongbuCerti || '-'}</span>
            </div>
            
            <div class="info-row">
                <span class="info-label">증권종류:</span>
                <span class="info-value">${certiType || '-'}</span>
            </div>
            
            <div class="info-row">
                <span class="info-label">요율:</span>
                <span class="info-value">
                    <select class="mobile-rate-select" data-id="${item.num}" id='mob-rate-${item.num}' 
                            onchange="handleRateChangeSj(this, '${item.num}', '${item.Jumin}', '${item.dongbuCerti}')">
                        <option value="-1">선택</option>
                        <option value="1" ${item.rate == 1 ? "selected" : ""}>1</option>
                        <option value="2" ${item.rate == 2 ? "selected" : ""}>0.9</option>
                        <option value="3" ${item.rate == 3 ? "selected" : ""}>0.925</option>
                        <option value="4" ${item.rate == 4 ? "selected" : ""}>0.898</option>
                        <option value="5" value="5" ${item.rate == 5 ? "selected" : ""}>0.889</option>
                       <option value="6" ${item.rate == 6 ? "selected" : ""}>1.074</option>
                       <option value="7" ${item.rate == 7 ? "selected" : ""}>1.085</option>
                       <option value="8" ${item.rate == 8 ? "selected" : ""}>1.242</option>
                       <option value="9" ${item.rate == 9 ? "selected" : ""}>1.253</option>
           ...
           <div class="info-row">
               ...
               <span class="info-value">${pushText || '-'}</span>
           </div>
           
           <div class="info-row">
               <span class="info-label">배서처리:</span>
               <span class="info-value">
                   <select class="mobile-status-select" data-id="${item.num}" id='mob-status-${item.num}'
                           onchange="handleStatusChangeSj(this, '${item.num}', '${item.push}')">
                       <option value="1" ${item.ch == 1 ? "selected" : ""}>미처리</option>
                       <option value="2" ${item.ch == 2 ? "selected" : ""}>처리</option>
                   </select>
               </span>
           ...
           <div class="info-row">
               <span class="info-label">보험료:</span>
               <span class="info-value">${formattedMothlyPremium}</span>
           </div>
           
           <div class="info-row">
               <span class="info-label">c보험료:</span>
               <span class="info-value">${formattedInsurancePremium}</span>
           </div>
           
           <div class="info-row">
               <span class="info-label">중복여부:</span>
               <span class="info-value">${item.duplNum || '-'}</span>
           </div>
       </div>
   `;
}
...
// 모바일 페이지네이션 업데이트 함수
function updateMobilePagination(currentPage, totalPages) {
   const mobilePaginationElement = document.getElementById('kje-mobile-pagination');
   ...
   if (currentPage > 1) {
       prevButton.addEventListener('click', (e) => {
           e.preventDefault();
           kjEndorseloadTable(currentPage - 1, currentCertiValue, currentDaeriValue, currentPushSangtaeValue);
       });
   }
   ...
   for (let i = startPage; i <= endPage; i++) {
       ...
       pageButton.addEventListener('click', (e) => {
           e.preventDefault();
           kjEndorseloadTable(i, currentCertiValue, currentDaeriValue, currentPushSangtaeValue);
       });
...
// 현재 상황 업데이트 함수
function updateCurrentSituation(pushCounts) {
   const statusText = `청약:${pushCounts.subscription},해지:${pushCounts.termination},계:${pushCounts.total}`;
   
   const currentSituation = document.getElementById('currentSituation');
   const mobileCurrentSituation = document.getElementById('currentSituation-mobile');
   
   if (currentSituation) currentSituation.innerHTML = statusText;
   if (mobileCurrentSituation) mobileCurrentSituation.innerHTML = statusText;
}

// f_kjEndorseloadTable 함수 추가
function f_kjEndorseloadTable() {
   const isMobile = window.innerWidth < 768;
   let pushValue;
   
   if (isMobile) {
       pushValue = document.getElementById('pushSangtaeList-mobile').value;
       document.getElementById('pushSangtaeList').value = pushValue;
   } else {
       pushValue = document.getElementById('pushSangtaeList').value;
       if (document.getElementById('pushSangtaeList-mobile')) {
           document.getElementById('pushSangtaeList-mobile').value = pushValue;
       }
   }
   
   kjEndorseloadTable(1, currentCertiValue, currentDaeriValue, pushValue);
}
...
@media (max-width: 767px) {
    ...
    .kje-mobile-card .card-header h6 {
        margin: 0;
        color: #333;
    }
    
    .kje-mobile-card .card-body {
        padding: 15px;
    }
    
    .kje-mobile-card .info-row {
        display: flex;
    ...
    .kje-mobile-card .info-row:last-child {
        border-bottom: none;
        margin-bottom: 0;
    }
    
    .kje-mobile-card .info-label {
        font-weight: bold;
        color: #555;
        flex: 0 0 120px;
    }
    
    .kje-mobile-card .info-value {
        text-align: right;
        color: #333;
        flex: 1;
    }
    ...
    /* 모바일 버튼 영역 - 데스크탑과 동일한 배경색 적용 */
    .kje-right-area {
        width: 100%;
        display: flex;
        gap: 5px;
        padding: 10px;
        /* 데스크탑과 동일한 배경색 제거 (기본 배경 사용) */
    }
    ...
    .mobile-pagination .page-link:hover:not(.active):not(.disabled) {
        background-color: #e9ecef;
    }
    
    /* 모바일에서 리스트 헤더 레이아웃 조정 */
    .kje-list-header {
        flex-direction: column;
    }
...
document.addEventListener('DOMContentLoaded', function() {
   // 반응형 처리를 위한 리사이즈 이벤트 리스너
   let resizeTimer;
   window.addEventListener('resize', function() {
       clearTimeout(resizeTimer);
       resizeTimer = setTimeout(function() {
           // 현재 화면 크기에 맞게 컨텐츠 재조정
           const isMobile = window.innerWidth < 768;
           if (!isMobile) {
               // 데스크탑으로 전환 시 모바일 셀렉트 값을 데스크탑으로 동기화
               syncSelectValues('mobile', 'desktop');
           } else {
               // 모바일로 전환 시 데스크탑 셀렉트 값을 모바일로 동기화
               syncSelectValues('desktop', 'mobile');
           }
       }, 250);
   });
});
...
// 셀렉트 값 동기화 함수
function syncSelectValues(from, to) {
   const fromSuffix = from === 'mobile' ? '-mobile' : '';
   const toSuffix = to === 'mobile' ? '-mobile' : '';
   
   const pushSangtaeFrom = document.getElementById('pushSangtaeList' + fromSuffix);
   const pushSangtaeTo = document.getElementById('pushSangtaeList' + toSuffix);
   
   const certiFrom = document.getElementById('certiList' + fromSuffix);
   const certiTo = document.getElementById('certiList' + toSuffix);
   
   const daeriFrom = document.getElementById('daeriCompanyList' + fromSuffix);
   const daeriTo = document.getElementById('daeriCompanyList' + toSuffix);
   
   if (pushSangtaeFrom && pushSangtaeTo) {
       pushSangtaeTo.value = pushSangtaeFrom.value;
   }
   
   if (certiFrom && certiTo) {
       certiTo.value = certiFrom.value;
   }
   
   if (daeriFrom && daeriTo) {
       daeriTo.value = daeriFrom.value;
   }
}
// kjRenderPagination 함수도 약간 수정 (가운데 정렬을 위해)
...
async function handleDamdangjarClick(dNum, memberNum){
  ...
  try {
    ...
    // 서버 API 호출
    const response = await fetch('../kj/api/kjDaeri/memberList.php', {
      method: "POST",
...
function seleSS(type, dNum, memberNum) {
  ...
  fetch('../kj/api/kjDaeri/updateDamdangja.php', {
    method: "POST",
    body: formData
  })
  .then(response => response.json())
...
async function handleIdClick(dNum) {
  try {
    ...
    // 서버 API 호출
    const response = await fetch('../kj/api/kjDaeri/id_.php', {
      method: "POST",
      body: formData
    });
...
function smsE_send() {
  ...
  // 서버로 데이터 전송 (fetch 사용)
  fetch('../kj/api/kjDaeri/smsSend.php', {
    method: 'POST',
    body: formData
  })
  .then(response => response.json())
...
function validateAndCheckId(inputElement) {
    ...
    // 서버로 아이디 중복 확인 요청 전송
    fetch('../kj/api/kjDaeri/checkUserId.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
...
function updateReadStatus(selectElement, num){
  ...
  // 서버로 권한 업데이트 요청 전송
  fetch('../kj/api/kjDaeri/updateReadIs.php', {
    method: 'POST',
    body: formData
  })
  .then(response => response.json())
...
function updatePermitStatus(selectElement, num) {
  ...
  // 서버로 권한 업데이트 요청 전송
  fetch('../kj/api/kjDaeri/updatePermit.php', {
    method: 'POST',
    body: formData
  })
  .then(response => response.json())
...
async function id_store(dNum,phone,company) {
  try {
    ...
    // API 호출
    const response = await fetch('../kj/api/kjDaeri/id_save.php', {
      method: 'POST',
      body: formData
    });
...
function validateAndUpdatePhone(inputElement, num) {
    ...
    // 서버로 비밀번호 업데이트 요청 전송
    fetch('../kj/api/kjDaeri/updatePhone.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
...
function validateAndUpdateUser(inputElement, num) {
    ...
    // 서버로 비밀번호 업데이트 요청 전송
    fetch('../kj/api/kjDaeri/updateUser.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
...
function validateAndUpdatePassword(inputElement, num) {
    ...
    // 서버로 비밀번호 업데이트 요청 전송
    fetch('../kj/api/kjDaeri/updatePassword.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
...
//문자 발송함수 
async function sms(phone, gisaName,dmemberNum) {
  // 이벤트 기본 동작 방지
  if (event) {
  ...
  try {
    ...
    const formData = new FormData();
    formData.append('phone', phone);
	formData.append('dMemberNum',dmemberNum);//DaeriMember 의 Num
	formData.append('sort',4);//DaeriMember 의 Num
    
    // 서버 API 호출
    const response = await fetch('../kj/api/kjDaeri/smsSearch.php', {
      method: "POST",
      body: formData // FormData 자동으로 Content-Type 설정
    });
...
async function sendSms() {
   ...
   try {
        ...
        const response = await fetch(`../kj/api/kjDaeri/smsSend.php`, {
            method: "POST",
        ...
        if (result.success) {
            ...
            alert(result.error || "문자 발송 중 오류 발생.");
        }
    } catch (error) {
...
async function kjEndorse(endorse_day, dNum, cNum, pNum) { 
	...
	document.getElementById('smsContents').value='';
    try {
        const response = await fetch(`../kj/api/kjDaeri/get_endorse_details.php?endorse_day=${endorse_day}&dNum=${dNum}&cNum=${cNum}&pNum=${pNum}`);
...
async function handleProgress(selectElement, num){
    ...
    try {
        ...
        const response = await fetch(`../kj/api/kjDaeri/changeProgress.php`, {
            method: "POST",
            body: formData,
...
async function endorseDateChange(endorse_day, dNum, cNum, pNum) {
    ...
    try {
        ...
        const response = await fetch(`../kj/api/kjDaeri/get_2012CertiTable_details2.php?cNum=${cNum}`);
        const data = await response.json();
        if (!data.success) {
            alert(data.error);
            return;
...
async function endorseDateSend(endorse_day, dNum, cNum, pNum) {
    ...
    if (!after_day) {
     ...
     try {
        ...
        const response = await fetch(`../kj/api/kjDaeri/endoreDateChange.php`, {
            method: "POST",
            body: formData,
          });
  ...
  try {
    ...
    if (result.success && result.data && result.data.length > 0) {
      ...
      result.data.forEach((item, index) => {
        ...
        // 행 추가 (서버 기준 순번 계산)
        const rowNumber = (currentPage - 1) * result.perPage + index + 1;
        m_smsList += `
          <tr>
            <td>${rowNumber}</td>
            <td>${formattedDate}</td>
...
async function handleRateChangeSj(selectElement, num, Jumin,policyNum) {
    ...
    try {
        ...
        const response = await fetch(`../kj/api/kjDaeri/changeRate.php`, {
            method: "POST",
            body: formData,
            signal: controller.signal
        });
...
async function handleStatusChangeSj(selectElement, num, push) {
    ...
    try {
        ...
        const response = await fetch(`../kj/api/kjDaeri/changeEndorse.php`, {
            method: "POST",
            body: formData,
            signal: controller.signal
        });
...
function handleStatusChangeKj(selectElement, num) {
    ...
    function startProcessing() {
        ...
        // 데이터 수집
        const push = document.getElementById(`pushP1-${num}`)?.value || '';
...
async function kjDaeriCompany(dNum) {
    try {
        ...
        if (typeof dNum === 'string' && /^\d{6}-\d{7}$/.test(dNum)) {
            // 주민번호로 회사 정보 조회
            const checkResponse = await fetch(`../kj/api/kjDaeri/dNum_check_jumin.php?jumin=${encodeURIComponent(dNum)}`);
        ...
        const response = await fetch(`../kj/api/kjDaeri/get_DaeriCompany_details.php?dNum=${dNum}`);
        const data = await response.json();
...
function dateChanged(dNum) {
    ...
    fetch('../kj/api/kjDaeri/firstDate.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
...
async function checkJuminFormat(event) {
    ...
    if (event.key === 'Enter') {
        ...
        try {
            ...
            // 서버에 주민번호로 기존 회사 조회
            const response = await fetch(`../kj/api/kjDaeri/dNum_check_jumin.php?jumin=${encodeURIComponent(juminValue)}`);
            const data = await response.json();
...
function daeriCompany_store(dNum) {
    ...
    try {
        ...
        // API 요청
        fetch('../kj/api/kjDaeri/dNumStore.php', {
            method: 'POST',
            headers: {
...
async function certi_store(cNum,dNum){
	...
	if(cNum=='new'){//신규인 경우
		...
		try {
			// API 호출
			const response = await fetch(`../kj/api/kjDaeri/sujeong.php?cNum=${cNum}&dNum=${dNum}&insurance=${insurance}&startDay=${startDay}&policyNum=${policyNum}&nabang=${nabang}`);
			const data = await response.json();
		...
		try {
			// API 호출
			const response = await fetch(`../kj/api/kjDaeri/sujeong.php?cNum=${cNum}&insurance=${insurance}&startDay=${startDay}&policyNum=${policyNum}&nabang=${nabang}`);
			const data = await response.json();
...
async function gitaChange(cNum) {
    ...
    try {
        // API 호출
        const response = await fetch(`../kj/api/kjDaeri/gitaChange.php?cNum=${cNum}&gita=${selectedValue}`);
        const data = await response.json();
...
async function nabangChange(cNum) {
    ...
    try {
        // API 호출
        const response = await fetch(`../kj/api/kjDaeri/nabangChange.php?cNum=${cNum}&nabang=${selectedValue}`);
        const data = await response.json();
...
function saveNew(cNum,dNum,gita,InsuraneCompany,endorseDay,policyNum) {
    ...
    // Form 데이터로 API 호출
    fetch("../kj/api/kjDaeri/save_new_data_encryption.php", {
        method: "POST",
        body: form
    })
    .then(response => {
...
function saveEndorse(cNum,dNum,gita,InsuraneCompany,endorseDay,policyNum) {
	...
	form.append('userName',SessionManager.getUserInfo().name);
    // Form 데이터로 API 호출
    fetch("../kj/api/kjDaeri/save_endorse_data_encryption.php", {
        method: "POST",
        body: form
    })
    .then(response => {
...
async function setMonthlyPremium(cNum) {
    ...
    try {
        const response = await fetch(`../kj/api/kjDaeri/get_2012CertiTable_details.php?cNum=${cNum}`);
        const data = await response.json();
...
function savePremiumData(cNum) {
    ...
    // Form 데이터로 API 호출
    fetch("../kj/api/kjDaeri/save_premium_data.php", {
        method: "POST",
        body: form
    })
    .then(response => {
...
async function togglePaymentMethod(event, cNum) {
   ...
   let divi='';
    ...
    try {
        // API 호출
        const response = await fetch(`../kj/api/kjDaeri/diviChange.php?cNum=${cNum}&divi=${divi}`);
        const data = await response.json();
