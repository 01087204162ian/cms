...
function damdangaLoadSearchTable(){
	...
	const sj='sj_'
	// API 요청
	fetch(`../kj/api/kjDaeri/damdangaList.php?sj=${sj}`)
    .then(response => {
      if (!response.ok) {
...
function dNumCompanyTable(page = 1, damdangja = 'all', duDay = '', dNumCompany = '') {
    ...
    if (!dNumCompany || dNumCompany === '') {
        ...
        dNumCompany = dNumCompanyElement ? dNumCompanyElement.value : '';
    }
    ...
    formData.append('currentInwonElement', currentInwonElement);
    formData.append('dNumCompany', dNumCompany);

    fetch('../kj/api/kjDaeri/dNumList.php', {
        method: 'POST',
        body: formData,
    })
        .then(response => {
...
function  giSaCertiRoda(dNum){
	...
	// API 요청
	fetch(`../kj/api/kjDaeri/giSaCertiList.php?dNum=${dNum}`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
...
function gisaSearch2() {
	...
	// API 요청
	fetch(`../kj/api/kjDaeri/gisaSearch.php?sj=${sj}&gijunDay=${gijunDay}&gisa_name=${gisa_name}&gisaPushSangtae=${gisaPushSangtae}`)
		.then(response => {
			if (!response.ok) {
				throw new Error(`HTTP error! Status: ${response.status}`);
			}
...
function gisaList(dNum, dongbuCerti,sort) {
    ...
    const url = new URL('../kj/api/kjDaeri/gisaList.php', window.location.origin);
...
function endorseDay(dNum) {
  ...
  // API 요청
  fetch(`../kj/api/kjDaeri/endorseDaySearch.php?dNum=${dNum}`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
...
function MonthlyEndorseSearch(dNum) {
    ...
    // API 요청
    fetch(`../kj/api/kjDaeri/gisaMonthlyEndorseSearch.php?dNum=${dNum}&lastMonthDueDate=${lastMonthDueDate}&thisMonthDueDate=${thisMonthDueDate}`)
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
            }
...
function updateSettlement(seqNo, status, customerName) {
    ...
    fetch('../kj/api/kjDaeri/updateSettlement.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
...
function premiumExcel(dNum) {
    ...
    newForm.method = 'post';
    newForm.action = '../kj/api/kjDaeri/gisaListExcel.php';
...
function memoPremiumSearch(jumin) {
    ...
    fetch('../kj/api/kjDaeri/memoSearch.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
...
function premiumSearch(dNum,lastMonthDueDate,thisMonthDueDate){
  ...
  // API 요청
  fetch(`../kj/api/kjDaeri/gisaListAdjustment.php?dNum=${dNum}&lastMonthDueDate=${lastMonthDueDate}&thisMonthDueDate=${thisMonthDueDate}`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
...
function adjustmentPremium(inputElement, dNum, thisMonthDueDate) {
    ...
    // 서버에 데이터 전송
    fetch('../kj/api/kjDaeri/AdjustmentPremiumSave.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
...
function settleSearch(attempted){
    ...
    // 서버에 데이터 전송
    fetch('../kj/api/kjDaeri/AdjustmentSettleSearch.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
...
function getPremium(inputElement, id) {
    ...
    // 서버에 데이터 전송
    fetch('../kj/api/kjDaeri/AdjustmentGetPreium.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
