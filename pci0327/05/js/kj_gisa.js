				...
				<table class="kje-data-table">
					<thead>
						<tr>
                            ...
                            <th class="col-11">보험료료</th>
							<th class="col-12">등록일</th>
							<th class="col-13">해지일</th>
							<th class="col-14">사고</th>
							<th class="col-15">인증번호</th>
 ...
 //const pushType={"1":"청약","2":"해지","3":"청약거절","4":"정상","5":"해지취소","6":"청약취소"}
            ...
            if (item.push == 4) { 
				if(item.cancel === 42){  // '42' → 42로 변경
				 ...
				 if (item.cancel === 12) {  // '12' → 12로 변경
					pushOptions = '해지'+item.cancel ;
				} else if (item.cancel === 13) {  // '13' → 13로 변경
...
const formattedAdjustedInsuranceMothlyPremium = item.AdjustedInsuranceMothlyPremium ? parseFloat(item.AdjustedInsuranceMothlyPremium).toLocaleString("en-US") : "";
        tableHTML += `
        <tr>
            <td >${rowNumber}</td>
            ...
            <td class="kje-preiminum">${formattedAdjustedInsuranceMothlyPremium }</td>
...
function validateGisaUpdateName(inputElement, num) {
    ...
    // 서버로 핸드폰 업데이트 요청 전송
    fetch('../kj/api/kjDaeri/updateGisaName.php', {
        method: 'POST',
        body: formData
    })
...
function validateGisaUpdatePhone(inputElement, num) {
    ...
    // 서버로 핸드폰 업데이트 요청 전송
    fetch('../kj/api/kjDaeri/updateGisaPhone.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
...
async function updateHaeji(element, DariMemberNum, dNum, cNum, policyNum, InsuranceCompany) {
    ...
    try {
        ...
        const response = await fetch(`../kj/api/kjDaeri/updateHaeji.php`, {
            method: "POST",
            body: formData
        });
