...
function kjPolicyloadSearchTable() {
	...
	// API 요청
	fetch(`../kj/api/kjDaeri/policySearch.php?sj=${sj}&fromDate=${fromDate}&toDate=${toDate}`)
		.then(response => {
			if (!response.ok) {
...
async function policyNumDetail(num){ 
 ...
 formData.append('num', num);
  ...
  try {
    // API 요청 전송
    const response = await fetch(`../kj/api/kjDaeri/policyNumDetail.php`, {
      method: "POST",
      body: formData,
    });
...
async function InsurancePremiumStatistics(certi) {
  ...
  try {
    const response = await fetch(`../kj/api/kjDaeri/PolicyNumInsurancePremiumStatistics.php?certi=${certi}`);
...
async function p_preminumInsert(certi){
	...
	try {
        const response = await fetch(`../kj/api/kjDaeri/get_kj_insurance_premium_data.php?certi=${certi}`);
        const data = await response.json();
...
function saveIPremiumData(certi) {
    ...
    // Form 데이터로 API 호출
    fetch("../kj/api/kjDaeri/save_Ipremium_data.php", {
        method: "POST",
        body: form
    })
    .then(response => {
