// 페이지네이션 관련 변수
let currentPage = 1;
const rowsPerPage = 20; // 한 페이지에 표시할 행 수
let totalPages = 0;
let currentData = [];
...
let isDriverSearchCalled = false;

// driverSearch 함수 수정 (driverSearch.js 파일에 적용)
function driverSearch() {
  // 강제 초기화 로직 확인

  isDriverSearchCalled=true;
  ...
  fetch('./api/customer/driver_data.php', {
    method: 'POST',
    body: formData
  })
  .then(response => {
...
function updateDriverUI(data) {
  ...
  if (searchCountElem && totalCountElem) {
    totalCountElem.textContent = currentData.length;
